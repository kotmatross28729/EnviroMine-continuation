...
public class EnviroPotion extends Potion {
    ...
    public static void RegisterPotions() {
        EnviroPotion.frostbite = ((EnviroPotion) new EnviroPotion(EM_Settings.frostBitePotionID, true, 8171462)
            .setPotionName("potion.enviromine.frostbite")).setIconIndex(0, 0);
        EnviroPotion.dehydration = ((EnviroPotion) new EnviroPotion(EM_Settings.dehydratePotionID, true, 3035801)
            .setPotionName("potion.enviromine.dehydration")).setIconIndex(1, 0);
        EnviroPotion.insanity = ((EnviroPotion) new EnviroPotion(EM_Settings.insanityPotionID, true, 5578058)
            .setPotionName("potion.enviromine.insanity")).setIconIndex(2, 0);
        EnviroPotion.heatstroke = ((EnviroPotion) new EnviroPotion(
            EM_Settings.heatstrokePotionID,
            true,
            RenderAssist.getColorFromRGBA(255, 0, 0, 255)).setPotionName("potion.enviromine.heatstroke"))
                .setIconIndex(3, 0);
        EnviroPotion.hypothermia = ((EnviroPotion) new EnviroPotion(EM_Settings.hypothermiaPotionID, true, 8171462)
            .setPotionName("potion.enviromine.hypothermia")).setIconIndex(4, 0);
    }
    ...
    public static void checkAndApplyEffects(EntityLivingBase entityLiving) {
        ...
        boolean isVampire = entityLiving instanceof EntityPlayer
            && EnviroUtils.isPlayerAVampire((EntityPlayer) entityLiving);
        // boolean isWerewolf = entityLiving instanceof EntityPlayer &&
        // EnviroUtils.isPlayerAWerewolf((EntityPlayer)entityLiving);
        boolean isCurrentlyAndroid = entityLiving instanceof EntityPlayer
            && EnviroUtils.isPlayerCurrentlyMOAndroid((EntityPlayer) entityLiving);
        boolean isCurrentlyWerewolf = entityLiving instanceof EntityPlayer
            && EnviroUtils.isPlayerCurrentlyWitcheryWerewolf((EntityPlayer) entityLiving);
        boolean isCurrentlyWolf = entityLiving instanceof EntityPlayer
            && EnviroUtils.isPlayerCurrentlyWitcheryWolf((EntityPlayer) entityLiving);
        int vampireLevel = entityLiving instanceof EntityPlayer
            ? EnviroUtils.getWitcheryVampireLevel((EntityPlayer) entityLiving)
            : 0;
        int werewolfLevel = entityLiving instanceof EntityPlayer
            ? EnviroUtils.getWitcheryWerewolfLevel((EntityPlayer) entityLiving)
            : 0;

        int vampireDuration = MathHelper.clamp_int(
            200 - (entityLiving instanceof EntityPlayer && EM_Settings.witcheryVampireImmunities ? vampireLevel : 0)
                * 15,
            0,
            200);
        int werewolfDuration = MathHelper.clamp_int(
            200 - (entityLiving instanceof EntityPlayer && EM_Settings.witcheryWerewolfImmunities ? werewolfLevel : 0)
                * 15,
            0,
            200);
        ...
        if (entityLiving.isPotionActive(heatstroke)) {
            ...
            // Don't inflict further effects if the stat in question is not tracked
                ...
                && !(EM_Settings.matterOverdriveAndroidImmunities && isCurrentlyAndroid)) {
                    if (effect.getAmplifier() >= 2 && entityLiving.getRNG()
                        .nextInt(2) == 0) {
                ...
                && !(EM_Settings.witcheryWerewolfImmunities && (isCurrentlyWerewolf || isCurrentlyWolf))) {
                    if (effect.getAmplifier() >= 2 && entityLiving.getRNG()
                        ...
                        entityLiving.attackEntityFrom(
                            EnviroDamageSource.organfailure,
                            4.0F * 5F
                                / (((EM_Settings.witcheryWerewolfImmunities && entityLiving instanceof EntityPlayer)
                                    ? werewolfLevel
                                    : 0) + 5));
                    }
        ...
        if (entityLiving.isPotionActive(frostbite)) {
            ...
            // Don't inflict further effects if the stat in question is not tracked
                ...
                && !(EM_Settings.witcheryWerewolfImmunities && (isCurrentlyWerewolf || isCurrentlyWolf))) {
                    if (entityLiving.getRNG()
                        .nextInt(2) == 0 && effect.getAmplifier() >= 2) {
                        entityLiving.attackEntityFrom(
                            EnviroDamageSource.frostbite,
                            4.0F * 5F
                                / (((EM_Settings.witcheryWerewolfImmunities && entityLiving instanceof EntityPlayer)
                                    ? werewolfLevel
                                    : 0) + 5));
                    ...
                    if (entityLiving.getHeldItem() != null) {
                        if (effect != null) {
                            if (entityLiving.getRNG()
                                .nextInt(
                                    20 + (entityLiving instanceof EntityPlayer && EM_Settings.witcheryVampireImmunities
                                        ? vampireLevel * 4
                                        : 0)
                                        + (entityLiving instanceof EntityPlayer
                                            && EM_Settings.witcheryWerewolfImmunities ? werewolfLevel * 4 : 0))
                                == 0) {
        ...
        if (entityLiving.isPotionActive(dehydration.id)) {
            ...
            // Don't inflict further effects if the stat in question is not tracked
            else if (EM_Settings.enableHydrate
                && (!EM_Settings.dimensionProperties.containsKey(entityLiving.dimension)
                    || (EM_Settings.dimensionProperties.containsKey(entityLiving.dimension)
        ...
        if (entityLiving.isPotionActive(insanity.id)) {
            ...
            // Don't inflict further effects if the stat in question is not tracked
            else if (EM_Settings.enableSanity
                && (!EM_Settings.dimensionProperties.containsKey(entityLiving.dimension)
                    || (EM_Settings.dimensionProperties.containsKey(entityLiving.dimension)
                ...
                && !(EM_Settings.witcheryVampireImmunities && isVampire)) {
                    ...
                    chance += (entityLiving instanceof EntityPlayer && EM_Settings.witcheryWerewolfImmunities
                        ? werewolfLevel * 4
                        : 0);
                    ...
                    if (EnviroMine.proxy.isClient() && effect.getAmplifier() >= 2
                        && entityLiving.getRNG()
                            .nextInt(
                                1000 + (entityLiving instanceof EntityPlayer && EM_Settings.witcheryWerewolfImmunities
                                    ? werewolfLevel * 200
                                    : 0))
                            == 0) {
                        displayFakeDeath();
                    ...
                    String sound = "";
                    if (entityLiving.getRNG()
                        .nextInt(chance) == 0 && entityLiving instanceof EntityPlayer) {
                        switch (entityLiving.getRNG()
                            .nextInt(16)) {
                            case 0 -> {
                        ...
                        float rndX = (player.getRNG()
                            .nextInt(6) - 3) * player.getRNG()
                                .nextFloat();
                        float rndY = (player.getRNG()
                            .nextInt(6) - 3) * player.getRNG()
                                .nextFloat();
                        float rndZ = (player.getRNG()
                            .nextInt(6) - 3) * player.getRNG()
                                .nextFloat();
                        ...
                        S29PacketSoundEffect packet = new S29PacketSoundEffect(
                            ...
                            1.0F,
                            !EM_Settings.randomizeInsanityPitch ? 1F
                                : player.getRNG()
                                    .nextBoolean() ? 0.2F
                                        : (player.getRNG()
                                            .nextFloat()
    ...
    private static void displayFakeDeath() {
        if (Minecraft.getMinecraft().currentScreen == null) {
            Minecraft.getMinecraft()
                .displayGuiScreen(new EM_GuiFakeDeath());
        }
    }
