...
public class SaveController {
    ...
    protected static final String dirName = Minecraft.getMinecraft().mcDataDir + File.separator
        + "config"
        + File.separator
        + "enviromine";
    ...
    public static boolean loadConfig(String name, String dirName) {
        ...
        if (!file.exists()) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.warn(
                "Config load canceled, file (" + file.getAbsolutePath()
                    + ") does not exist. This is normal for first run.");
            return false;
        } else {
        ...
        try {
            ...
            LOADED_VERSION = nbt.getCompoundTag(UISettingsData)
                .getString("CONFIG_VERSION");
    ...
    public static void saveConfig(String name, String dirName) {
        ...
        if (!dir.exists() && !dir.mkdirs()) throw new ReportedException(
            new CrashReport("Unable to create the configuration directories", new Throwable()));
