...
public class Gui_EventManager {
    ...
    public void renderevent(InitGuiEvent.Post event) {
        ...
        if (event.gui instanceof GuiIngameMenu && !EM_Settings.voxelMenuExists) {
            ...
            try {
                byte b0 = -16;
                enviromine = new GuiButton(
                    1348,
                    width / 2 - 100,
                    height / 4 + 24 + b0,
                    StatCollector.translateToLocal("options.enviromine.menu.title"));
                // enviromine = new EM_Button(1348, width / 2 - 100, height / 4 + 24 + b0,
                // StatCollector.translateToLocal("options.enviromine.menu.title") , newPost);
                ...
                // enviromine = new GuiButton(1348, width - 175, height - 30, 160, 20,
                // StatCollector.translateToLocal("options.enviromine.menu.title") + newPost);
                enviromine = new GuiButton(
                    1348,
                    width - 175,
        ...
        if (event.gui instanceof GuiIngameMenu) {
            if (event.button.id == enviromine.id) {
                Minecraft.getMinecraft()
                    .displayGuiScreen(new EM_Gui_Menu(event.gui));
            }
    ...
    public static final ResourceLocation guiResource = new ResourceLocation(
        "enviromine",
        "textures/gui/status_Gui.png");
    public static final ResourceLocation blurOverlayResource = new ResourceLocation(
        "enviromine",
        "textures/misc/blur.png");
    ...
    public void onGuiRender(RenderGameOverlayEvent.Post event) {
        ...
        if (tracker == null) {
            if (!(EM_Settings.enableAirQ == false && EM_Settings.enableBodyTemp == false
                && EM_Settings.enableHydrate == false
                && EM_Settings.enableSanity == false)) {
                // Minecraft.getMinecraft().fontRenderer.drawStringWithShadow("NO ENVIRONMENT DATA", xPos, (height -
                // yPos) - 8, 16777215);
            ...
            for (HudItem huditem : HUDRegistry.getActiveHudItemList()) {
                ...
                if (mc.thePlayer.ridingEntity instanceof EntityLivingBase) {
                    if (huditem.shouldDrawOnMount()) {
                        ...
                        // GL11.glScalef((float)UI_Settings.guiScale, (float)UI_Settings.guiScale,
                        // (float)UI_Settings.guiScale);
