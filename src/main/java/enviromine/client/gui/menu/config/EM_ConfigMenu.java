...
public class EM_ConfigMenu extends GuiConfig {
    ...
    public EM_ConfigMenu(GuiScreen parentScreen) {
        super(parentScreen, getMainElements(), EM_Settings.MOD_ID, false, false, EM_Settings.MOD_NAME, " ");
    }

    public EM_ConfigMenu(List<IConfigElement> list, Configuration config) {
        super(null, list, EM_Settings.MOD_ID, false, false, EM_Settings.MOD_NAME, " ");
        tempConfigs.clear();
    ...
    private static List<IConfigElement> getMainElements() {
        ...
        // File profilSettings = new File(EM_ConfigHandler.loadedProfile +
        // EnviroMine.theWorldEM.getProfile()+"_Settings.cfg");
        ...
        list.add(
            new DummyCategoryElement(
                "Profile Settings",
                "editor.enviromine.settings",
                getConfigElements(profilSettings)));
        ...
        list.add(
            new DummyCategoryElement("Stability Types", "editor.enviromine.stability", getConfigElements(stabFile)));

        File[] customFiles = new File(EM_ConfigHandler.loadedProfile + EM_ConfigHandler.customPath).listFiles();
        list.add(
            new DummyCategoryElement("Custom Configs", "editor.enviromine.custom", getConfigElements(customFiles)));
    ...
    private static List<IConfigElement> getConfigElements(File... files) {
        ...
        for (File entry : files) {
            if (entry.getName()
                .contains(".DS_Store")) continue;

            Configuration config = new Configuration(entry, true);
            tempConfigs.add(config);
            Iterator<String> iterator = config.getCategoryNames()
                .iterator();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawCenteredString(
            ...
            I18n.format("editor.enviromine.currentProfile") + ": " + EM_ConfigHandler.getProfileName(),
            this.width / 2,
            18,
            16777215);
