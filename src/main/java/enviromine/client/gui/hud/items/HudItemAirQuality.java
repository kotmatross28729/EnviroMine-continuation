...
public class HudItemAirQuality extends HudItem
{
	...
	public void render()
	{
		...
		if(
				...
				&& !EM_Settings.dimensionProperties.get(ecplayermp.dimension).trackAirQuality)
		...
		if(UI_Settings.ShowText == true && !rotated)
		{
			...
			//Render Text
			//RenderAssist.drawTexturedModalRect(getTextPosX(), posY, 64, getHeight() * 4, 32, getHeight());
			Minecraft.getMinecraft().fontRenderer.drawString(String.format("%.1f", Gui_EventManager.tracker.airQuality) + "%", getTextPosX(), posY, 16777215);
		}
		GL11.glPopMatrix();
	}
