...
public class HudItemSanity extends HudItem	{
	...
	public void render() 
	{
		...
		if(
				...
				&& !EM_Settings.dimensionProperties.get(ecplayermp.dimension).trackSanity)
		...
		if(UI_Settings.ShowText == true  && !this.rotated)
		{
				...
				//Render Text
				//RenderAssist.drawTexturedModalRect(getTextPosX(), posY, 64, getHeight() * 4, 32, getHeight());
				Minecraft.getMinecraft().fontRenderer.drawString(String.format("%.1f", Gui_EventManager.tracker.sanity) + "%", getTextPosX(), posY, 16777215);
		}
		GL11.glPopMatrix();
	}
