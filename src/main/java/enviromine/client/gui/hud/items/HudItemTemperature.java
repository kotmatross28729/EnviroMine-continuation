...
public class HudItemTemperature extends HudItem
{
	...
	public void render()
	{
		...
		if(
				...
				&& !EM_Settings.dimensionProperties.get(ecplayermp.dimension).trackTemp)
...
//		float dispHeat = new BigDecimal(String.valueOf(Gui_EventManager.tracker.bodyTemp)).setScale(3, RoundingMode.DOWN).floatValue();
//		float FdispHeat = new BigDecimal(String.valueOf((Gui_EventManager.tracker.bodyTemp * 1.8) + 32)).setScale(2, RoundingMode.DOWN).floatValue();
		...
		if(UI_Settings.ShowText == true && !this.rotated)
		{
			...
			{
				Minecraft.getMinecraft().fontRenderer.drawString(String.format("%.1f", Gui_EventManager.tracker.bodyTemp * 1.8 + 32) + "F", getTextPosX(), posY, 16777215);
			} else
			{
				Minecraft.getMinecraft().fontRenderer.drawString(String.format("%.1f", Gui_EventManager.tracker.bodyTemp) + "C", getTextPosX(), posY, 16777215);
			}
		}
