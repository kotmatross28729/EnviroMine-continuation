...
public class HudItemTemperature extends HudItem {
    ...
    public void render() {
        ...
        if ((EM_Settings.dimensionProperties.containsKey(ecplayermp.dimension)
            && !EM_Settings.dimensionProperties.get(ecplayermp.dimension).trackTemp) || !EM_Settings.enableBodyTemp
        ...
        int preheatBar = MathHelper
            .ceiling_float_int(((Gui_EventManager.tracker.airTemp + 50) / 150) * this.getWidth());
        int preheatIco = 16 - MathHelper.ceiling_float_int(((Gui_EventManager.tracker.airTemp + 50) / 150) * 16);

        // float dispHeat = new BigDecimal(String.valueOf(Gui_EventManager.tracker.bodyTemp)).setScale(3,
        // RoundingMode.DOWN).floatValue();
        // float FdispHeat = new BigDecimal(String.valueOf((Gui_EventManager.tracker.bodyTemp * 1.8) + 32)).setScale(2,
        // RoundingMode.DOWN).floatValue();
        ...
        if (UI_Settings.ShowGuiIcons == true) {
            ...
            if (preheatIco >= 8) {
                RenderAssist
                    .drawTexturedModalRect(iconPosX, posY - 4 + preheatIco, 16, 96 + preheatIco, 16, 16 - preheatIco);
            } else {
                RenderAssist
                    .drawTexturedModalRect(iconPosX, posY - 4 + preheatIco, 0, 96 + preheatIco, 16, 16 - preheatIco);
            }
        ...
        if (UI_Settings.ShowText == true && !this.rotated) {
            ...
            if (UI_Settings.useFarenheit) {
                Minecraft.getMinecraft().fontRenderer.drawString(
                    String.format("%.1f", Gui_EventManager.tracker.bodyTemp * 1.8 + 32) + "F",
                    getTextPosX(),
                    posY,
                    16777215);
            } else {
                Minecraft.getMinecraft().fontRenderer.drawString(
                    String.format("%.1f", Gui_EventManager.tracker.bodyTemp) + "C",
                    getTextPosX(),
                    posY,
                    16777215);
            }
    ...
    public void renderScreenOverlay(int scaledwidth, int scaledheight) {
        ...
        if (Gui_EventManager.tracker.bodyTemp >= 39) {
            ...
            RenderAssist
                .drawScreenOverlay(scaledwidth, scaledheight, RenderAssist.getColorFromRGBA(255, 255, 255, grad));
            ...
            RenderAssist
                .drawScreenOverlay(scaledwidth, scaledheight, RenderAssist.getColorFromRGBA(125, 255, 255, grad));
        }
