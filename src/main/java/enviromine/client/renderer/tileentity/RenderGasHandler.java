...
public class RenderGasHandler implements ISimpleBlockRenderingHandler
{
	...
	public boolean renderWorldBlock(IBlockAccess blockAccess, int i, int j, int k, Block oBlock, int modelId, RenderBlocks renderer)
	{
		...
		{
			if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.ALL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Trying to render gas without block at position!");
			return false;
		} else if(blockAccess.getTileEntity(i, j, k) == null)
		{
			if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.ALL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Trying to render gas without tile at position!");
			return false;
		}
