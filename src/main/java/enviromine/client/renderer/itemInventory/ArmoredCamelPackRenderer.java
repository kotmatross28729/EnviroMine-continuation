...
public class ArmoredCamelPackRenderer implements IItemRenderer
{
	...
	public void renderItem(ItemRenderType type, ItemStack itemStack, Object... data)
	{
		...
		if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey(EM_Settings.IS_CAMEL_PACK_TAG_KEY))
		{
			renderFillBar(itemStack);
		} else if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY))
		{
			...
			//  	String text = Integer.toString(itemStack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY));
	...
	public void renderFillBar(ItemStack itemStack)
	{
		...
		int height = 10;
		int width = 1;
		int fill = itemStack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY);
		int max = itemStack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY);
		int disp = (fill <= 0 ? 0 : fill > max ? 100 : (int)(fill / (max / 100F)));
		
		//double currentFill = itemStack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY);
