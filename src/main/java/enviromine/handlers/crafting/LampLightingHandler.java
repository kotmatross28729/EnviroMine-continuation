...
public class LampLightingHandler {
    // I feel as though making a whole class for this is probably overkill but it keeps with the organisation of
    // crafting handlers - Funwayguy
    ...
    public void onCrafting(PlayerEvent.ItemCraftedEvent event) {
        ...
        if (hasLamp && lighter != null && lighter.getItemDamage() < lighter.getMaxDamage()) {
            if (!event.player.inventory
                .addItemStackToInventory(new ItemStack(Items.flint_and_steel, 1, lighter.getItemDamage() + 1))) {
                event.player.dropPlayerItemWithRandomChoice(
                    new ItemStack(Items.flint_and_steel, 1, lighter.getItemDamage() + 1),
                    false);
            }
        }
    }
