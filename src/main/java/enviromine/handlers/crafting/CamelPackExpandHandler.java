...
public class CamelPackExpandHandler implements IRecipe
{
	...
	public boolean matches(InventoryCrafting inv, World world)
	{
		...
		for (int i = inv.getSizeInventory() - 1; i >= 0; i--)
		{
			...
			{
				if (item.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY) > EM_Settings.camelPackMax) {
	...
	public ItemStack getCraftingResult(InventoryCrafting inv)
	{
		...
		pack.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, pack1.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY) + pack2.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY));
		pack.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY, pack1.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY) + pack2.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY));
		pack.getTagCompound().setBoolean(EM_Settings.IS_CAMEL_PACK_TAG_KEY, true);
