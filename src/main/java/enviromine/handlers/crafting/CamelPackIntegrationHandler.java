		...
		for (int i = inv.getSizeInventory() - 1; i >= 0; i--)
		{
			...
			} else if (item.getItem() instanceof ItemArmor && ((ItemArmor)item.getItem()).armorType == 1)
			{
				...
				if (EM_Settings.armorProperties.containsKey(name) && EM_Settings.armorProperties.get(name).allowCamelPack)
				{
					...
					{
						if (item.hasTagCompound() && item.stackTagCompound.hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY))
						{
							if (hasPack)
							{
								return false;
		...
		if (armor != null)
		{
			if (isRemove)
			{
				...
				if (obj instanceof Item) {
					...
					out.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, armor.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY));
					out.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY, armor.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY));
					out.getTagCompound().setBoolean(EM_Settings.IS_CAMEL_PACK_TAG_KEY, true);
			...
			} else
			{
				...
				armor.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, pack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY));
				armor.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY, pack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY));
		...
		if (this.matches((InventoryCrafting)craftMatrix, event.player.worldObj)) {
			if (isRemove) {
				for (int i = craftMatrix.getSizeInventory() - 1; i >= 0; i--)
				{
					...
					{
						continue;
					} else if (slot.hasTagCompound() && slot.getTagCompound().hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY))
					{
						slot.stackSize++;
						slot.getTagCompound().removeTag(EM_Settings.CAMEL_PACK_FILL_TAG_KEY);
						slot.getTagCompound().removeTag(EM_Settings.CAMEL_PACK_MAX_TAG_KEY);
