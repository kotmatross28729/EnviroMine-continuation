...
public class CamelPackRefillHandler implements IRecipe {
    ...
    public boolean matches(InventoryCrafting inv, World world) {
        ...
        for (int i = inv.getSizeInventory() - 1; i >= 0; i--) {
            ...
            ItemProperties itemProps = EM_Settings.itemProperties
                .get(Item.itemRegistry.getNameForObject(item.getItem()) + "," + item.getItemDamage());
            itemProps = itemProps != null ? itemProps
                : EM_Settings.itemProperties.get(Item.itemRegistry.getNameForObject(item.getItem()));
            ...
            if (item.hasTagCompound() && item.getTagCompound()
                ...
                if (pack != null) {
                    ...
                    pack = item.copy();
                    // packFillCur = item.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY);
                    // packFillMax = item.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY);
                }
            } else if (itemProps != null && itemProps.camelFill != 0) {
                Item outItem = (Item) Item.itemRegistry.getObject(itemProps.fillReturnItem);
                ItemStack outStack = null;
                if (outItem != null) {
                    outStack = new ItemStack(
                        outItem,
                        1,
                        itemProps.fillReturnMeta < 0 ? item.getItemDamage() : itemProps.fillReturnMeta);
                }
    ...
    public ItemStack getCraftingResult(InventoryCrafting inv) {
        ...
        if (totalFill < 0) {
            int attemptedFill = pack.getTagCompound()
                .getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY) + totalFill;

            if (attemptedFill <= pack.getTagCompound()
                .getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY) && attemptedFill >= 0) {
                return emptyItem;
            } else {
                return null;
            }
            ...
            if (attemptedFill <= pack.getTagCompound()
                .getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY) && attemptedFill >= 0) {
                pack.getTagCompound()
                    .setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, attemptedFill);
                return pack;
            } else {
                return null;
            }
    ...
    public void onCrafting(PlayerEvent.ItemCraftedEvent event) {
        ...
        if (this.matches((InventoryCrafting) craftMatrix, event.player.worldObj)) {
            if (!craftMatrix.getInventoryName()
                .equals("container.crafting")) {
                return;
            } else {
                for (int i = craftMatrix.getSizeInventory() - 1; i >= 0; i--) {
                    ...
                    if (slot == null) {
                        ...
                        .hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY) && totalFill < 0) {
                            slot.stackSize += 1;
                            slot.getTagCompound()
                                .setInteger(
