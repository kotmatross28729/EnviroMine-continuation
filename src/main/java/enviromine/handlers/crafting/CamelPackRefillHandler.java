...
public class CamelPackRefillHandler implements IRecipe
{
	...
	public boolean matches(InventoryCrafting inv, World world)
	{
		...
		for (int i = inv.getSizeInventory() - 1; i >= 0; i--)
		{
			...
			if (item.hasTagCompound() && item.getTagCompound().hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY))
			{
				if (pack != null)
				{
					return false;
	...
	public ItemStack getCraftingResult(InventoryCrafting inv)
	{
		...
		if (totalFill < 0)
		{
			int attemptedFill = pack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY) + totalFill;
			
			if (attemptedFill <= pack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY) && attemptedFill >= 0)
			{
				return emptyItem;
			} else
			{
		...
		} else
		{
			...
			if (attemptedFill <= pack.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY) && attemptedFill >= 0)
			{
				pack.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, attemptedFill);
				return pack;
			} else
			{
				return null;
		...
		if (this.matches((InventoryCrafting)craftMatrix, event.player.worldObj))
		{
			...
			} else
			{
				for (int i = craftMatrix.getSizeInventory() - 1; i >= 0; i--)
				{
					...
					{
						continue;
					} else if (slot.hasTagCompound() && slot.getTagCompound().hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY) && totalFill < 0)
					{
						slot.stackSize += 1;
						slot.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, slot.getTagCompound().getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY) + totalFill);
