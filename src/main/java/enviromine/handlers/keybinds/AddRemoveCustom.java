...
public class AddRemoveCustom {
    ...
    public static void doAddRemove() {
        ...
        if ((!(Minecraft.getMinecraft()
            .isSingleplayer()) || !EnviroMine.proxy.isClient()) && Minecraft.getMinecraft().thePlayer != null) {
            ...
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                try {
                    ...
                    if (mc.thePlayer.getHeldItem() != null) {
                        Item item = mc.thePlayer.getHeldItem()
                            .getItem();
                        int itemMeta = mc.thePlayer.getHeldItem()
                            .getItemDamage();
                        String idName = Item.itemRegistry.getNameForObject(item);
                        String name = mc.thePlayer.getHeldItem()
                            .getDisplayName();

                        if (item instanceof ItemArmor) {
                            Minecraft.getMinecraft()
                                .displayGuiScreen(new GuiAddCustom(item));
                    ...
                    if (type.name() == "ENTITY") {
                        ...
                        if (EntityList.getEntityID(lookingAt) > 0) {
                            ...
                            .lookupModSpawn(lookingAt.getClass(), false) != null) {
                                id = EntityRegistry.instance()
                                    .lookupModSpawn(lookingAt.getClass(), false)
                                    .getModEntityId() + 128;
                            } else {
                                mc.thePlayer.addChatMessage(
                                    new ChatComponentText(
                                        "Failed to add config entry. " + lookingAt.getCommandSenderName()
                                            + " has no ID!"));
                        ...
                        if (EM_ConfigMenu.ElementExist(lookingAt, EntityProperties.base)) {
                            ...
                            mc.thePlayer.addChatMessage(
                                new ChatComponentText(
                                    lookingAt.getCommandSenderName() + " (" + id + ") " + returnValue));
                        }
                        ...
                        int blockMeta = Minecraft.getMinecraft().thePlayer.worldObj
                            .getBlockMetadata(blockX, blockY, blockZ);
                    ...
                    if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger
                        .log(Level.WARN, "A NullPointerException occured while adding config entry!", e);
                }
            } else {
