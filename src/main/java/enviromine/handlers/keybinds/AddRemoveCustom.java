	...
	public static void doAddRemove()
	{
		...
		if(mc.currentScreen == null)
		{
			if(Keyboard.isKeyDown(Keyboard.KEY_LSHIFT))
			{
				try
				{
					...
					if(type.name() == "ENTITY")
					{
						...
						} else
						{
							...
							if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.WARN, "Failed to add config entry. " + lookingAt.getCommandSenderName() + " has no ID!");
						}
				...
				{
					if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.WARN, "A NullPointerException occured while adding config entry!", e);
				}
			}
			else
			{
