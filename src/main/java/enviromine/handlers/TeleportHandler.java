...
public class TeleportHandler extends Teleporter {
    ...
    public static TeleportHandler GetInstance(WorldServer world) {
        return GetInstance(world, true); // Changed to true to prevent being given elevator blocks. --AstroTibs
    }
    ...
    public static TeleportHandler GetInstance(WorldServer world, boolean recall) {
        ...
        if (tele != null) {
            // If there are any elevators in this dimension
            tele.recall = recall;
            return tele;
        } else {
            // If there are no elevators in this dimension
    ...
    public void placeInPortal(Entity entity, double par2, double par4, double par6, float par8) {
        if (!this.placeInExistingPortal(entity, par2, par4, par6, par8)) {
            ...
            if (entity instanceof EntityPlayer) {
                ...
                // The below block explicitly places the elevator block items into your hand on activation, or onto the
    ...
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8) {
        ...
        if (this.destinationCoordinateCache.containsItem(j1)) {
            ...
            brkAll: for (k1 = l - short1; k1 <= l + short1; ++k1) {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
                    ...
                    for (int i2 = 255 - 1; i2 >= 0; --i2) {
                        if (this.worldServerInstance.getBlock(k1, i2, l1) == ObjectHandler.elevator
                            && this.worldServerInstance.getBlock(k1, i2 + 1, l1) == ObjectHandler.elevator) {
                            if (this.worldServerInstance.getBlockMetadata(k1, i2, l1) == 1
                                && this.worldServerInstance.getBlockMetadata(k1, i2 + 1, l1) == 0) {
                                // Normal elevator
                                // recall = false;
                            } else if (this.worldServerInstance.getBlockMetadata(k1, i2, l1) == 2
                                && this.worldServerInstance.getBlockMetadata(k1, i2 + 1, l1) == 3) {
                                    // Recall
                                    recall = true;
                                } else {
                                    continue;
        ...
        if (d3 >= 0.0D) {
            if (flag) {
                this.destinationCoordinateCache
                    .add(j1, new PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
    ...
    public boolean makePortal(Entity par1Entity) {
        int i = (int) (MathHelper.floor_double(par1Entity.posX)
            + Math.signum(par1Entity.posX < 0 ? par1Entity.posX : 0));
        int j = 5;// MathHelper.floor_double(par1Entity.posY);
        int k = (int) (MathHelper.floor_double(par1Entity.posZ)
            + Math.signum(par1Entity.posZ < 0 ? par1Entity.posZ : 0));
        ...
        if (this.worldServerInstance.provider.dimensionId == EM_Settings.caveDimID) {
            for (int checkH = 120; checkH >= 32; checkH--) {
                if (this.worldServerInstance.isAirBlock(i, checkH, k)
                    && this.worldServerInstance.isAirBlock(i, checkH + 1, k)
                    && this.worldServerInstance.getBlock(i, checkH - 1, k)
                        .isNormalCube()) {
                    j = checkH;
                    break;
                }
            ...
            for (int x = i - 1; x <= i + 1; x++) {
                for (int y = j - 1; y <= j + 2; y++) {
                    for (int z = k - 1; z <= k + 1; z++) {
                        if (y == j - 1) {
                            if (!this.worldServerInstance.getBlock(x, y, z)
                                .isOpaqueCube());
                            {
                                ...
                                if (x != i && z != k) {
                                    ...
                                    while (!this.worldServerInstance.getBlock(x, supY, z)
                                        .isOpaqueCube() && supY >= 0) {
    ...
    public static boolean RecallElevator(int x, int y, int z, boolean invert) {
        WorldServer caveWorld = MinecraftServer.getServer()
            .worldServerForDimension(EM_Settings.caveDimID);
        ...
        brkAll: for (i = x - short1; i <= x + short1; ++i) {
            for (k = z - short1; k <= z + short1; ++k) {
                for (j = 255 - 1; j >= 0; --j) {
                    if (checkWorld.getBlock(i, j, k) == ObjectHandler.elevator
                        && checkWorld.getBlockMetadata(i, j, k) == 1
                        && checkWorld.getBlock(i, j + 1, k) == ObjectHandler.elevator
                        && checkWorld.getBlockMetadata(i, j + 1, k) == 0) {
