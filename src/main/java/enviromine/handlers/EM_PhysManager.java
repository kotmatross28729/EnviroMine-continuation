...
public class EM_PhysManager {
    ...
    public static void schedulePhysUpdate(World world, int x, int y, int z, boolean updateSelf, String type) {
        if (world.isRemote || world.getTotalWorldTime() < worldStartTime + EM_Settings.worldDelay) {
            ...
            if (chunkDelay.get(world.provider.dimensionId + "" + (x >> 4) + "," + (z >> 4))
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                for (int k = -1; k <= 1; k++) {
                    ...
                    String position = (new StringBuilder()).append(x + i)
                        .append(",")
                        .append(y + j)
                        .append(",")
                        .append(z + k)
    ...
    public static void callPhysUpdate(World world, int x, int y, int z, Block block, int meta, String type) {
        String position = (new StringBuilder()).append(x)
            .append(",")
            .append(y)
            .append(",")
            .append(z)
        ...
        if (excluded.containsKey(position)) {
            if (!excluded.get(position)
                .equals("Collapse") && type.equals("Collapse")) {
                excluded.put(position, type);
            } else {
                return;
            }
        ...
        if (chunk != null) {
            waterLogged = (chunk.getBiomeGenForWorldCoords(x & 15, z & 15, world.getWorldChunkManager()).rainfall > 0
                && world.isRaining()
                && world.canBlockSeeTheSky(x, y + 1, z)) || touchingWater;
        }
        ...
        if (EM_Settings.blockProperties.containsKey("" + Block.blockRegistry.getNameForObject(block) + "," + meta)
            || EM_Settings.blockProperties.containsKey("" + Block.blockRegistry.getNameForObject(block))) {
            if (EM_Settings.blockProperties
                .containsKey("" + Block.blockRegistry.getNameForObject(block) + "," + meta)) {
                blockProps = EM_Settings.blockProperties
                    .get("" + Block.blockRegistry.getNameForObject(block) + "," + meta);
            } else {
           ...
           * else if(block instanceof BlockFalling || ((block == Blocks.dirt || block == Blocks.snow || block ==
           * Blocks.snow_layer) && (waterLogged || touchingWater)))
           * {
        ...
        if (validSlideType && EM_Settings.enableLandslide) {
            ...
            if (emptyBelow) {
                if (!(block instanceof BlockFalling) && !usedSlidePositions.contains("" + pos[0] + "," + pos[2])) {
                    ...
                    EntityPhysicsBlock physBlock = new EntityPhysicsBlock(
                        world,
                        pos[0] + 0.5,
                        pos[1] + 0.5,
                        pos[2] + 0.5,
            ...
            } else if (!(pos[0] == npos[0] && pos[1] == npos[1] && pos[2] == npos[2])
                && !usedSlidePositions.contains("" + npos[0] + "," + npos[2])) {
        ...
        if (isLegalType(world, x, y, z) && blockNotSolid(world, x, y - 1, z, false) && blockData[4] <= 0) {
            ...
            if (blockProps != null) {
                ...
                if (blockProps.dropName.equals("")) {
                    ...
                    dropNum = 0;
                } else if (Item.getItemFromBlock(Block.getBlockFromName(blockProps.dropName)) != null
                    && blockProps.dropNum > 0) {
                        dropType = 2;
            ...
            if (!defaultDrop) {} else if (dropBlock == null || dropBlock == Blocks.air
            ...
            boolean supported = hasSupports(
                world,
                x,
                y,
                z,
                (touchingWaterDirect && EM_Settings.waterCollapse) ? MathHelper.floor_double(supportDist / 2D)
                    : supportDist);
            // missingBlocks total = 25 - 26

            if (missingBlocks > 0 && blockNotSolid(world, x, y - 1, z, false) && !supported) {
                if (!world.isRemote && ((missingBlocks > minThreshold
                    && (world.rand.nextInt(dropChance) == 0 || type.equals("Collapse")))
                    || missingBlocks >= maxThreshold
                    || ((touchingWaterDirect && EM_Settings.waterCollapse) && world.rand.nextBoolean()))) {
                    if (dropType == -1) {
                        world.playAuxSFX(
                            2001,
                            x,
                            y,
                            z,
                            Block.getIdFromBlock(block) + (world.getBlockMetadata(x, y, z) << 12));
                        ...
                        if (block == Blocks.ice) {
                            Material mat = world.getBlock(x, y - 1, z)
                                .getMaterial();
                    ...
                    if (isCustom && dropMeta > -1) {
                        entityphysblock = new EntityPhysicsBlock(
                            world,
                            (float) x + 0.5F,
                            (float) y + 0.5F,
                            (float) z + 0.5F,
                    ...
                    // world.playAuxSFX(2001, x, y, z, Block.getIdFromBlock(entityphysblock.block) +
                    // (entityphysblock.meta << 12));
                    world.playSoundEffect(
                        (double) ((float) x + 0.5F),
                        (double) ((float) y + 0.5F),
                        (double) ((float) z + 0.5F),
                        entityphysblock.block.stepSound.func_150496_b(),
                        (entityphysblock.block.stepSound.getVolume() + 1.0F) / 2.0F,
                        entityphysblock.block.stepSound.getPitch() * 1.2F);
        ...
        for (int i = x - 1; i <= x + 1; i++) {
            for (int j = y - 1; j <= y + 1; j++) {
                for (int k = z - 1; k <= z + 1; k++) {
                    ...
                    if (EM_Settings.blockProperties.containsKey("" + name)
                        || EM_Settings.blockProperties.containsKey("" + name + "," + metaID)) {
                    ...
                    if (world
                        ...
                        data[0] += 1;
                    } else if ((blockNotSolid(world, i, j, k, false) || (world.getBlock(x, y, z)
                        .getMaterial() != Material.leaves && material == Material.leaves))
                        && !(i == x && j < y + 1 && k == z)) {
                            if (j < y + 1) {
                                data[1] += 1;
                            }
    ...
    public static boolean isLegalType(World world, int x, int y, int z) {
        ...
        if (EM_Settings.blockProperties.containsKey("" + name + "," + meta)
            || EM_Settings.blockProperties.containsKey("" + name)) {
            if (EM_Settings.blockProperties.containsKey("" + name + "," + meta)) {
            ...
            for (int i = updateNum - 1; i >= 0; i -= 1) {
                ...
                if (physSchedule.size() - 1 < i) {
                    if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                        Level.ERROR,
                        "Unable to get physcis schedule entry, index out of bounds! (Size: " + physSchedule.size()
                            + ", Index: "
                            + i
                            + ")");
                    canClear = false;
                ...
                if (((World) entry[0]).getChunkProvider()
                    .chunkExists((Integer) entry[1] >> 4, (Integer) entry[3] >> 4)) {
                    locLoaded = ((World) entry[0])
                        .getChunkFromChunkCoords((Integer) entry[1] >> 4, (Integer) entry[3] >> 4).isChunkLoaded;
                } else {
                    locLoaded = false;
                }
                ...
                if (locLoaded) {
                    ...
                    if (((String) entry[5]).equalsIgnoreCase("Slide")) {
                        String position = (new StringBuilder()).append((Integer) entry[1])
                            .append(",")
                            .append((Integer) entry[2])
                            .append(",")
                            .append((Integer) entry[3])
                            .toString();
                        if (!excluded.containsKey(position)) {
                        ...
                        updateSurroundingWithExclusions(
                            (World) entry[0],
                            (Integer) entry[1],
                            (Integer) entry[2],
                            (Integer) entry[3],
                ...
                if (physSchedule.size() - 1 >= i) {
                    ...
                    if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger
                        .log(Level.ERROR, "Failed to remove entry from physics schedule. Things may break badly!");
                }
    ...
    public static int[] slideDirection(World world, int[] pos, boolean checkEntities) {
        ...
        if (blockNotSolid(world, x + 1, y, z, true) && blockNotSolid(world, x + 1, y - 1, z, false)
            && (!checkEntities || world
                .getEntitiesWithinAABB(
                    EntityPhysicsBlock.class,
                    AxisAlignedBB.getBoundingBox(x + 1, y - 2, z, x + 2, y, z + 1))
                .size() <= 0)) {
            canSlideDir.add("X+");
        }
        if (blockNotSolid(world, x - 1, y, z, true) && blockNotSolid(world, x - 1, y - 1, z, false)
            && (!checkEntities || world
                .getEntitiesWithinAABB(
                    EntityPhysicsBlock.class,
                    AxisAlignedBB.getBoundingBox(x - 1, y - 2, z, x, y, z + 1))
                .size() <= 0)) {
            canSlideDir.add("X-");
        }
        if (blockNotSolid(world, x, y, z + 1, true) && blockNotSolid(world, x, y - 1, z + 1, false)
            && (!checkEntities || world
                .getEntitiesWithinAABB(
                    EntityPhysicsBlock.class,
                    AxisAlignedBB.getBoundingBox(x, y - 2, z + 1, x + 1, y, z + 2))
                .size() <= 0)) {
            canSlideDir.add("Z+");
        }
        if (blockNotSolid(world, x, y, z - 1, true) && blockNotSolid(world, x, y - 1, z - 1, false)
            && (!checkEntities || world
                .getEntitiesWithinAABB(
                    EntityPhysicsBlock.class,
                    AxisAlignedBB.getBoundingBox(x, y - 2, z - 1, x + 1, y, z))
                .size() <= 0)) {
            canSlideDir.add("Z-");
        }
