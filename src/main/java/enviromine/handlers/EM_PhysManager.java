...
public class EM_PhysManager
{
	...
	public static void callPhysUpdate(World world, int x, int y, int z, Block block, int meta, String type)
	{
		...
		if(isLegalType(world, x, y, z) && blockNotSolid(world, x, y - 1, z, false) && blockData[4] <= 0)
		{
			...
			boolean supported = hasSupports(world, x, y, z, (touchingWaterDirect && EM_Settings.waterCollapse)? MathHelper.floor_double(supportDist/2D) : supportDist);
			//missingBlocks total = 25 - 26

			if(missingBlocks > 0 && blockNotSolid(world, x, y - 1, z, false) && !supported)
			{
				if(!world.isRemote && ((missingBlocks > minThreshold && (world.rand.nextInt(dropChance) == 0 || type.equals("Collapse"))) || missingBlocks >= maxThreshold || ((touchingWaterDirect && EM_Settings.waterCollapse) && world.rand.nextBoolean())))
				{
					if(dropType == -1)
					{
	...
	public static void updateSchedule()
	{
		...
		{
			if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Physics updates exeeded 4096! Dumping update schedule");
			physSchedule.clear();
			return;
		}
		...
		if(currentTime >= EM_Settings.physInterval)
		{
			...
			for(int i = updateNum - 1; i >= 0; i -= 1)
			{
				...
				if(timer.elapsed(TimeUnit.SECONDS) > 2)
				{
					if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Physics updates are taking too long! Dumping schedule!");
					physSchedule.clear();
					canClear = false;
					break;
				}
				...
				{
					if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Unable to get physcis schedule entry, index out of bounds! (Size: " + physSchedule.size() + ", Index: " + i +")");
					canClear = false;
					break;
				}
				...
				{
					if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Physics schedule dumped early! Resetting scheduler...");
					physSchedule.clear();
					canClear = false;
					break;
				}
				...
				{
					if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Failed to remove entry from physics schedule. Things may break badly!");
				}
