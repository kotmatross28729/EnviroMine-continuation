        ...
        if (!EM_Settings.foodSpoiling || (rotProps == null && !(item.getItem() instanceof ItemFood)) || rotTime < 0) {
            ...
            if (UBD == 0) {
                ...
                if (rotProps == null) {
                    ...
                    return Item.itemRegistry.getObject(rotProps.rotID) == null ? null
                        : new ItemStack(
                            (Item) Item.itemRegistry.getObject(rotProps.rotID),
                            item.stackSize,
                            rotProps.rotMeta < 0 ? item.getItemDamage() : rotProps.rotMeta);
                }
            } else {
                item.getTagCompound()
                    .setLong("EM_ROT_TIME", rotTime);
