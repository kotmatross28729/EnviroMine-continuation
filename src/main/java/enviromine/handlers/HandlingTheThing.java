...
public class HandlingTheThing {
    ...
    public static void stalkPlayer(EntityPlayer player) {
        boolean flag = player.getEntityData()
            .getBoolean("EM_THING_TARGET")
            || (player.worldObj.getWorldTime() % 6000 == 0 && EM_Settings.thingChance > player.getRNG()
                .nextFloat());

        // Check if Halloween or Friday 13th. Guarantees attack if true!
        if ((date.get(Calendar.MONTH) == Calendar.OCTOBER && date.get(Calendar.DAY_OF_MONTH) == 31)
        ...
        if (player == null || !player.isEntityAlive()
            || !flag
            || player.dimension != EM_Settings.caveDimID
            || player.worldObj.difficultySetting == EnumDifficulty.PEACEFUL) {
            if (player != null && player.getEntityData() != null) {
                player.getEntityData()
                    .setBoolean("EM_THING_TARGET", false);
                player.getEntityData()
                    .setInteger("EM_THING", 0);
            }
            return;
        }

        player.getEntityData()
            .setBoolean("EM_THING_TARGET", true);
        ...
        int darkness = player.getEntityData()
            .getInteger("EM_THING");
        int deathSpeed = 1;
        ...
        if (player.worldObj.getBlockLightValue(i, j, k) < 10
            && player.worldObj.getSavedLightValue(EnumSkyBlock.Sky, i, j, k) < 10
    ...
    public static boolean hasWitnesses(EntityPlayer victim) {
        ...
        List<EntityPlayer> players = victim.worldObj
            .getEntitiesWithinAABB(EntityPlayer.class, victim.boundingBox.expand(128, 128, 128));
