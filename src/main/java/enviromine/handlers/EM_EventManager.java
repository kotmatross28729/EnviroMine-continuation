...
public class EM_EventManager {
    ...
    public void onEntityJoinWorld(EntityJoinWorldEvent event) {
        ...
        if (!event.world.isRemote) {
            if (EM_PhysManager.chunkDelay.containsKey(
                event.world.provider.dimensionId + ""
                    + (MathHelper.floor_double(event.entity.posX) >> 4)
                    + ","
                    + (MathHelper.floor_double(event.entity.posZ) >> 4))) {
                chunkPhys = (EM_PhysManager.chunkDelay.get(
                    event.world.provider.dimensionId + ""
                        + (MathHelper.floor_double(event.entity.posX) >> 4)
                        + ","
                        + (MathHelper.floor_double(event.entity.posZ) >> 4))
        ...
        } else if (event.entity instanceof EntityFallingBlock oldSand && !(event.entity instanceof EntityPhysicsBlock)
    ...
    public void onLivingSpawn(LivingSpawnEvent.CheckSpawn event) {
        if (EM_Settings.enforceWeights) {
            if (EnviroMine.caves.totalSpawnWeight > 0 && event.world.provider.dimensionId == EM_Settings.caveDimID
                && EM_Settings.caveSpawnProperties.containsKey(EntityList.getEntityID(event.entity))) {
    ...
    public void onLivingDeath(LivingDeathEvent event) {
        ...
        if (event.entityLiving instanceof EntityMob && event.source.getEntity() != null
            && event.source.getEntity() instanceof EntityPlayer player) {
            EnviroDataTracker tracker = EM_StatusManager.lookupTracker(player);

            if (player.isPotionActive(EnviroPotion.insanity) && player.getActivePotionEffect(EnviroPotion.insanity)
                .getAmplifier() >= 2) {
                int val = player.getEntityData()
                    .getInteger("EM_MIND_MAT") + 1;
                player.getEntityData()
                    .setInteger("EM_MIND_MAT", val);
            ...
            if (tracker != null && tracker.sanity < 100 && !(event.entityLiving instanceof EntityAnimal)) { // DON'T
                // TOUCH
    ...
    public void onEntityAttacked(LivingAttackEvent event) {
        ...
        if ((event.source == DamageSource.fallingBlock || event.source == DamageSource.anvil
        ...
        if (event.source == DamageSource.fallingBlock && event.entityLiving instanceof EntityPlayer) {
            event.entityLiving.getEntityData()
                .setLong("EM_SAFETY", event.entityLiving.worldObj.getTotalWorldTime());
        }

        if (event.source == EnviroDamageSource.gasfire && event.entityLiving instanceof EntityPlayer) {
            event.entityLiving.getEntityData()
                .setLong("EM_THAT", event.entityLiving.worldObj.getTotalWorldTime());
        }

        if (event.entityLiving instanceof EntityPlayer && event.entityLiving.getEntityData()
            .hasKey("EM_MIND_MAT")) {
            event.entityLiving.getEntityData()
                .removeTag("EM_MIND_MAT");
        }
    ...
    public void onEntityInteract(EntityInteractEvent event) {
        ...
        if (event.target instanceof EntityIronGolem && event.entityPlayer.getEquipmentInSlot(0) != null) {
            ...
            if (stack.getItem() == Items.name_tag && stack.getDisplayName()
                .toLowerCase()
                .equals("siyliss")) {
        ...
        if (event.target instanceof IInventory chest) {
            RotHandler.rotInvo(event.entityPlayer.worldObj, chest); // For every entity that is... inventory? (like
            // chest mobs?)
        }
    }
            ...
            if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                if (isWater) {
                    if (world.getBlockMetadata(
                        ...
                        Item newItem = ObjectHandler.getBucketFromWaterType(getWaterType(world, i, j, k))
                            .getItem();
                    ...
                    if (isCauldronHeatingBlock(world.getBlock(i, j - 1, k), world.getBlockMetadata(i, j - 1, k))) {
                        newItem = ObjectHandler.getBucketFromWaterType(WaterUtils.heatUp(getWaterType(world, i, j, k)))
                            .getItem();
                    }
        ...
        if (movingobjectposition != null) {
            if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                if (isWater || isValidCauldron) {
                    ...
                    if (isPolymer) {
                        newItem = ObjectHandlerCompat.getItemStackFromWaterType(getWaterType(world, i, j, k))
                            .getItem();
                    } else {
                        newItem = ObjectHandler.getItemStackFromWaterType(getWaterType(world, i, j, k))
                            .getItem();
                    }
                    ...
                    if (isValidCauldron
                        ...
                        if (isPolymer) {
                            newItem = ObjectHandlerCompat
                                .getItemStackFromWaterType(WaterUtils.heatUp(getWaterType(world, i, j, k)))
                                .getItem();
                        } else {
                    ...
                    if (isValidCauldron) {
                        player.worldObj
                            .setBlockMetadataWithNotify(i, j, k, player.worldObj.getBlockMetadata(i, j, k) - 1, 2);
                    } else if (EM_Settings.finiteWater) {
                        player.worldObj
                            .setBlock(i, j, k, Blocks.flowing_water, player.worldObj.getBlockMetadata(i, j, k) + 1, 2);
                    }
    ...
    public static void drinkWater(EntityPlayer entityPlayer, PlayerInteractEvent event) {
        ...
        if ((EM_Settings.dimensionProperties.containsKey(entityPlayer.dimension)
        ...
        if (mop != null) {
            ...
            if (mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                boolean isValidCauldron = (entityPlayer.worldObj.getBlock(i, j, k) == Blocks.cauldron
                    && entityPlayer.worldObj.getBlockMetadata(i, j, k) > 0);

                if (isWater || isValidCauldron) {
                    if (tracker != null && tracker.hydration < 100F) {
                        WaterUtils.WATER_TYPES type = WaterUtils.WATER_TYPES.CLEAN;

                        if (isValidCauldron && isCauldronHeatingBlock(
                            entityPlayer.worldObj.getBlock(i, j - 1, k),
                            entityPlayer.worldObj.getBlockMetadata(i, j - 1, k))) {
                        ...
                        int werewolfDuration200 = MathHelper
                            .clamp_int(200 - (EM_Settings.witcheryWerewolfImmunities ? werewolfLevel : 0) * 15, 0, 200);
                        int werewolfDuration600 = MathHelper
                            .clamp_int(600 - (EM_Settings.witcheryWerewolfImmunities ? werewolfLevel : 0) * 45, 0, 600);
                        ...
                        if (type.isDirty) {
                            if (!(EM_Settings.witcheryWerewolfImmunities
                                ...
                                if (entityPlayer.getRNG()
                                    .nextInt(2) == 0) {
                                    entityPlayer
                                        .addPotionEffect(new PotionEffect(Potion.hunger.id, werewolfDuration200));
                                }
                                if (entityPlayer.getRNG()
                                    .nextInt(4) == 0) {
                                    entityPlayer
                                        .addPotionEffect(new PotionEffect(Potion.poison.id, werewolfDuration200));
                                }
                            }
                        }
                            ...
                            if (!(EM_Settings.witcheryWerewolfImmunities
                                ...
                                if (entityPlayer.getActivePotionEffect(EnviroPotion.dehydration) != null
                                    ...
                                    entityPlayer.addPotionEffect(
                                        new PotionEffect(EnviroPotion.dehydration.id, werewolfDuration600, amp + 1));
                                } else {
                                    entityPlayer.addPotionEffect(
                                        new PotionEffect(EnviroPotion.dehydration.id, werewolfDuration600));
                                }
                            }
                        }
                        ...
                        if (isValidCauldron) {
                            entityPlayer.worldObj.setBlockMetadataWithNotify(
                                i,
                                j,
                                k,
                                entityPlayer.worldObj.getBlockMetadata(i, j, k) - 1,
                                2);
                        } else if (EM_Settings.finiteWater) {
                            entityPlayer.worldObj.setBlock(
                                i,
                                j,
                                k,
    ...
    public void onPlayerUseItem(PlayerUseItemEvent.Finish event) {
        ...
        if (EM_Settings.itemProperties.containsKey(Item.itemRegistry.getNameForObject(item.getItem()))
            ...
            if (EM_Settings.itemProperties
                .containsKey(Item.itemRegistry.getNameForObject(item.getItem()) + "," + item.getItemDamage())) {
                itemProps = EM_Settings.itemProperties
                    .get(Item.itemRegistry.getNameForObject(item.getItem()) + "," + item.getItemDamage());
            } else {
    ...
    public void onLivingUpdate(LivingUpdateEvent event) {
        ...
        if (event.entityLiving.worldObj.isRemote) {
            ...
            if (event.entityLiving instanceof EntityPlayer && event.entityLiving.worldObj.isRemote) {
                if (Minecraft.getMinecraft().thePlayer.isPotionActive(EnviroPotion.insanity)) {
                    int chance = 100 / (Minecraft.getMinecraft().thePlayer.getActivePotionEffect(EnviroPotion.insanity)
                        .getAmplifier() + 1);
        ...
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            if (EM_Settings.enableHighTempCooling) {
                EnviroDataTracker tracker = EM_StatusManager.lookupTracker(event.entityLiving);
                if (tracker != null && tracker.bodyTemp > EM_Settings.highTempThreshold) {
                    EntityPlayer player = (EntityPlayer) event.entityLiving;
                    if (player.isInsideOfMaterial(Material.water)) {
                        applyWaterCooling(player, tracker);
                    }
                }
            }
            ...
            AxisAlignedBB boundingBox = AxisAlignedBB
                .getBoundingBox(
                    event.entityLiving.posX - EM_Settings.DavyLampGasDetectRange,
                    event.entityLiving.posY - EM_Settings.DavyLampGasDetectRange,
                    event.entityLiving.posZ - EM_Settings.DavyLampGasDetectRange,
                    event.entityLiving.posX + EM_Settings.DavyLampGasDetectRange,
                    event.entityLiving.posY + EM_Settings.DavyLampGasDetectRange,
                    event.entityLiving.posZ + EM_Settings.DavyLampGasDetectRange)
                .expand(2D, 2D, 2D);
            if (event.entityLiving.worldObj.getEntitiesWithinAABB(TileEntityGas.class, boundingBox)
                .size() <= 0) {
                ReplaceInvoItems(
                    invo,
            ...
            if (event.entityLiving.worldObj.provider.dimensionId == EM_Settings.caveDimID
                ...
                if (distance > prePos[3]) {
                    prePos[3] = distance;
                    event.entityLiving.getEntityData()
                        .setIntArray("EM_CAVE_DIST", prePos);
                }
            }
            ...
            if (!event.entityLiving.isPotionActive(EnviroPotion.hypothermia)
                ...
                if (event.entityLiving.getEntityData()
                    ...
                    event.entityLiving.getEntityData()
                        .setLong("EM_WINTER", event.entityLiving.worldObj.getTotalWorldTime());
                }
            ...
            if (EM_Settings.enableAirQ && EM_Settings.enableBodyTemp
                ...
                && EM_Settings.enableLandslide
                && EM_Settings.enablePhysics
                && EM_Settings.enableQuakes) {
                int seaLvl = 48;

                if (EM_Settings.dimensionProperties.containsKey(event.entityLiving.worldObj.provider.dimensionId)) {
                    seaLvl = MathHelper.ceiling_double_int(
                        EM_Settings.dimensionProperties.get(event.entityLiving.worldObj.provider.dimensionId).sealevel
                            * 0.75F);
                ...
                if (event.entityLiving.posY < seaLvl) {
                    long time = event.entityLiving.getEntityData()
                        .getLong("EM_MINE_TIME");
                    long date = event.entityLiving.getEntityData()
                        .getLong("EM_MINE_DATE");
                    time += event.entityLiving.worldObj.getTotalWorldTime() - date;
                    event.entityLiving.getEntityData()
                        .setLong("EM_MINE_DATE", event.entityLiving.worldObj.getTotalWorldTime());
                    event.entityLiving.getEntityData()
                        .setLong("EM_MINE_TIME", time);
                ...
                if (event.entityLiving.getEntityData()
                    .hasKey("EM_MINE_TIME")) {
                    event.entityLiving.getEntityData()
                        .removeTag("EM_MINE_TIME");
                }
                if (event.entityLiving.getEntityData()
                    .hasKey("EM_MINE_DATE")) {
                    event.entityLiving.getEntityData()
                        .removeTag("EM_MINE_DATE");
                }
            }
        }
        ...
        if (event.entityLiving.isPotionActive(EnviroPotion.hypothermia)
            ...
            if (event.entityLiving.isPotionActive(EnviroPotion.frostbite)) {
                if (event.entityLiving.getActivePotionEffect(EnviroPotion.frostbite)
                    .getAmplifier() > 0) {
        ...
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            if (((EntityPlayer) event.entityLiving).isPlayerSleeping() && !event.entityLiving.worldObj.isDaytime()) {
                ...
                tracker.lastSleepTime = (int) event.entityLiving.worldObj.getWorldInfo()
                    .getWorldTime() % 24000;
            } else if (event.entityLiving.worldObj.isDaytime()) {
                int relitiveTime = (int) event.entityLiving.worldObj.getWorldInfo()
                    .getWorldTime() % 24000;
                ...
                if (tracker.sleepState.equals("Asleep") && tracker.lastSleepTime - relitiveTime > 100) {
                    ...
                    if (tracker.sanity + timeSlept > 100F) {
                        ...
                        if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                            Level.ERROR,
                            "Something went wrong while calculating sleep sanity gain! Result: " + timeSlept);
                        tracker.sanity = 100;
                        if (tracker.trackedEntity instanceof EntityPlayer) {
                            ((EntityPlayer) tracker.trackedEntity).addChatMessage(
                                new ChatComponentText(
                                    EnumChatFormatting.RED
                                        + "[ENVIROMINE] Sleep state failed to detect sleep time properly!"));
                            ((EntityPlayer) tracker.trackedEntity).addChatMessage(
                                new ChatComponentText(EnumChatFormatting.RED + "[ENVIROMINE] Defaulting to 100%"));
                        }
                    }
                }
        ...
        for (int i = 0; i < invo.getSizeInventory(); i++) {
            ...
            if (stack != null) {
                if (stack.getItem() == fItem && (stack.getItemDamage() == fDamage || fDamage <= -1)) {
                    invo.setInventorySlotContents(
                        i,
                        new ItemStack(rItem, stack.stackSize, fDamage <= -1 ? stack.getItemDamage() : rDamage));
                }
            }
        }
    }
    ...
    public void onLand(LivingFallEvent event) {
        if (event.entityLiving.getRNG()
            ...
            EM_PhysManager.schedulePhysUpdate(
                event.entityLiving.worldObj,
                MathHelper.floor_double(event.entityLiving.posX),
                MathHelper.floor_double(event.entityLiving.posY - 1),
                MathHelper.floor_double(event.entityLiving.posZ),
                true,
                "Jump");
        }
    ...
    public void onWorldLoad(Load event) {
        ...
        if (EM_Settings.worldDir == null && server.isServerRunning()) {
            if (EnviroMine.proxy.isClient()) {
                EM_Settings.worldDir = MinecraftServer.getServer()
                    .getFile("saves/" + server.getFolderName());
            } else {
    ...
    public void onChunkLoad(ChunkEvent.Load event) {
        ...
        if (!EM_PhysManager.chunkDelay.containsKey(
            event.world.provider.dimensionId + "" + event.getChunk().xPosition + "," + event.getChunk().zPosition)) {
            EM_PhysManager.chunkDelay.put(
                event.world.provider.dimensionId + "" + event.getChunk().xPosition + "," + event.getChunk().zPosition,
                event.world.getTotalWorldTime() + EM_Settings.chunkDelay);
        }
    }
                ...
                Minecraft.getMinecraft()
                    ...
                    .playSound(
                        new PositionedSoundRecord(
                            new ResourceLocation(event.name),
                            1.0F,
                            (event.entity.worldObj.rand.nextFloat() - event.entity.worldObj.rand.nextFloat()) * 0.2F
                                + 1.0F,
                            (float) event.entity.posX,
                            (float) event.entity.posY,
                            (float) event.entity.posZ));
            }
            event.setCanceled(true);
        }
    }
    ...
    public void onMusicPlay(PlaySoundEvent17 event) {
        if (Minecraft.getMinecraft().thePlayer != null && event.category == SoundCategory.MUSIC
            && Minecraft.getMinecraft().thePlayer.dimension == EM_Settings.caveDimID) {
        ...
        if (EM_Settings.enablePlayerRandomMobRender) {
            if (Minecraft.getMinecraft().thePlayer.isPotionActive(EnviroPotion.insanity)
                ...
                if (entity == null || entity.worldObj != event.entityPlayer.worldObj) {
                    BiomeGenBase biome = event.entityPlayer.worldObj.getBiomeGenForCoords(
                        MathHelper.floor_double(event.entityPlayer.posX),
                        MathHelper.floor_double(event.entityPlayer.posZ));
                    ArrayList<SpawnListEntry> spawnList = (ArrayList<SpawnListEntry>) biome
                        .getSpawnableList(EnumCreatureType.monster);

                    if (spawnList.size() <= 0) {
                        entity = new EntityZombie(event.entityPlayer.worldObj);
                    } else {
                        int spawnIndex = event.entityPlayer.getRNG()
                ...
                // GL11.glRotatef(180F - (event.entityPlayer.renderYawOffset + (event.entityPlayer.renderYawOffset -
                // event.entityPlayer.prevRenderYawOffset) * partialTicks), 0F, 1F, 0F);
    ...
    private EnumChatFormatting parseColor(String colorName, EnumChatFormatting defaultColor) {
    ...
    public void onItemTooltip(ItemTooltipEvent event) {
        if (event.itemStack != null) {
            ...
            if (ItemProperties.base.hasProperty(event.itemStack)) {
                ...
                float effSanity = EM_Settings.enableItemPropsDivideByTwo ? itemProps.effSanity * 2F
                    : itemProps.effSanity;
                float effHydration = EM_Settings.enableItemPropsDivideByTwo ? itemProps.effHydration * 2F
            ...
            if (event.itemStack.getItem() instanceof EnviroItemWaterBottle enviroItemWaterBottle) {
                ...
                if (waterType.isRadioactive) {
                    event.toolTip.add(radioactiveColor + "[" + I18n.format("enviromine.tooltip.isRadioactive") + "]");
                } else if (waterType.isDirty) {
                    event.toolTip.add(dirtyColor + "[" + I18n.format("enviromine.tooltip.isDirty") + "]");
                } else if (waterType.isSalty) {
                    event.toolTip.add(saltyColor + "[" + I18n.format("enviromine.tooltip.isSalty") + "]");
                }
            ...
            if (event.itemStack.hasTagCompound()) {
                if (event.itemStack.getTagCompound()
                    .hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY)) {
                    int fill = event.itemStack.getTagCompound()
                        .getInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY);
                    int max = event.itemStack.getTagCompound()
                        .getInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY);
                    if (fill > max) {
                        fill = max;
                        event.itemStack.getTagCompound()
                            .setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, fill);
                    }

                    int disp = fill <= 0 ? 0 : (int) ((float) fill / (float) max * 100);
                    event.toolTip.add(
                        new ChatComponentTranslation("misc.enviromine.tooltip.water", disp + "%", fill, max)
                            .getUnformattedText());
                }

                if (event.itemStack.getTagCompound()
                    .getLong("EM_ROT_DATE") > 0 && EM_Settings.foodSpoiling) {
                    double rotDate = event.itemStack.getTagCompound()
                        .getLong("EM_ROT_DATE");
                    double rotTime = event.itemStack.getTagCompound()
                        .getLong("EM_ROT_TIME");
                    ...
                    if (curTime - rotDate <= 0) {
                        event.toolTip.add(
                            new ChatComponentTranslation(
                                "misc.enviromine.tooltip.rot",
                                "0%",
                                MathHelper.floor_double((curTime - rotDate) / 24000L),
                                MathHelper.floor_double(rotTime / 24000L)).getUnformattedText());
                    } else {
                        event.toolTip.add(
                            new ChatComponentTranslation(
                                "misc.enviromine.tooltip.rot",
                                MathHelper.floor_double((curTime - rotDate) / rotTime * 100D) + "%",
                                MathHelper.floor_double((curTime - rotDate) / 24000L),
                                MathHelper.floor_double(rotTime / 24000L)).getUnformattedText());
                    }
                }
                ...
                if (event.itemStack.getTagCompound()
                    ...
                    int i = event.itemStack.getTagCompound()
                        .getInteger(EM_Settings.GAS_MASK_FILL_TAG_KEY);
                    int max = event.itemStack.getTagCompound()
                        .getInteger(EM_Settings.GAS_MASK_MAX_TAG_KEY);
                    int disp = (i <= 0 ? 0 : i > max ? 100 : (int) (i / (max / 100F)));
                    event.toolTip.add(
                        new ChatComponentTranslation("misc.enviromine.tooltip.filter", disp + "%", i, max)
                            .getUnformattedText());
                }
            }
        }
    ...
    // total life of food supplies
    {
        ...
        if (lowestDate >= 0) {
            ...
            event.crafting.getTagCompound()
                .setLong("EM_ROT_DATE", lowestDate);
            event.crafting.getTagCompound()
                .setLong("EM_ROT_TIME", rotTime);
        }
    }
    ...
    public static boolean isCauldronHeatingBlock(Block blockInWorld, int metaInWorld) {
        ...
        for (String cauldronBlock : EM_Settings.cauldronHeatingBlocks) {
            ...
            if (block != null && block == blockInWorld && (blockmeta < 0 || (blockmeta == metaInWorld))) {
                return true;
            }
        }
    ...
    private static void applyWaterCooling(EntityPlayer player, EnviroDataTracker tracker) {
        World world = player.worldObj;
        AxisAlignedBB box = player.boundingBox; // player's current collision box
        ...
        float bestTargetTemp = -1; // keep -1 if no water found

        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    Block block = world.getBlock(x, y, z);
                    // ignore air and non-water blocks
                    ...
                    // vanilla water (including flowing water)
                    if (block == Blocks.water || block == Blocks.flowing_water) {
                        targetTemp = EM_Settings.coolTempNormal;
                    }
                    // custom water
                    else if (block instanceof BlockEnviroMineWater) {
                        Fluid fluid = ((BlockEnviroMineWater) block).getFluid();
                        WaterUtils.WATER_TYPES type = WaterUtils.getTypeFromFluid(fluid);
                        String typeName = type.name();
                        if (typeName.contains("FROSTY")) {
                            targetTemp = EM_Settings.coolTempFrosty;
                        } else if (typeName.contains("COLD")) {
                            targetTemp = EM_Settings.coolTempCold;
                        } else {
                            targetTemp = EM_Settings.coolTempNormal;
                        }
                    }
                    ...
                    if (targetTemp >= 0) {
                        // choose the coldest water (lowest target temperature)
                        if (bestTargetTemp < 0 || targetTemp < bestTargetTemp) {
        ...
        // if usable water was found, set body temperature
        if (bestTargetTemp >= 0) {
