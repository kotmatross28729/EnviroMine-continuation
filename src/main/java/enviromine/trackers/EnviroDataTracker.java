...
//			isWerewolf = EnviroUtils.isPlayerAWerewolf((EntityPlayer)trackedEntity);
		...
		if (trackedEntity.isPotionActive(Potion.fireResistance) && bodyTemp > 37F) {bodyTemp = 37F;} // IF you have fire resistance, ambient temperature can never "feel" higher than 37
...
//					}
				...
				else if(sanity <= 25F - (EM_Settings.witcheryWerewolfImmunities ? werewolfLevel : 0))
				{
					trackedEntity.addPotionEffect(new PotionEffect(EnviroPotion.insanity.id, werewolfDuration, 1));
				}
				else if(sanity <= 50F - (EM_Settings.witcheryWerewolfImmunities ? werewolfLevel*2 : 0))
				{
