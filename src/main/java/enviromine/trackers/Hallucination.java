...
public class Hallucination {
    ...
    public Hallucination(EntityLivingBase entityLiving) {
        ...
        BiomeGenBase biome = entityLiving.worldObj
            .getBiomeGenForCoords(MathHelper.floor_double(x), MathHelper.floor_double(z));
        ...
        int spawnIndex = entityLiving.getRNG()
            .nextInt(spawnList.size());

        try {
            falseEntity = (EntityLiving) spawnList.get(spawnIndex).entityClass
                .getConstructor(new Class[] { World.class })
                .newInstance(new Object[] { entityLiving.worldObj });
        ...
        if (falseEntity.worldObj.isRemote && falseEntity instanceof EntityLiving) {
            ...
            falseEntity.getEntityData()
                .setBoolean("EM_Hallucination", true);
    ...
    private List<EntityPlayer> getPlayerList(EntityLivingBase entity) {
        ...
        while (ite.hasNext()) {
            ...
            if (e instanceof EntityPlayer && !e.getCommandSenderName()
                .equals(entity.getCommandSenderName()) && !isPlayerSeenWrong(e.getCommandSenderName())) {
    ...
    public void doOverride() {
        if (this.type == Type.OVERRIDE) {
            this.falseEntity.setPositionAndRotation(
                this.overriding.posX,
                this.overriding.posY,
                this.overriding.posZ,
                this.overriding.rotationYaw,
                this.overriding.rotationPitch);
        }
    }
    ...
    public static boolean isAtValidSpawn(EntityLivingBase creature) {
        return creature.worldObj.checkNoEntityCollision(creature.boundingBox)
            && creature.worldObj.getCollidingBoundingBoxes(creature, creature.boundingBox)
                .isEmpty()
            && !creature.worldObj.isAnyLiquid(creature.boundingBox)
            && isValidLightLevel(creature);
    }
    ...
    protected static boolean isValidLightLevel(EntityLivingBase creature) {
        ...
        if (creature.worldObj.getSavedLightValue(EnumSkyBlock.Sky, i, j, k) > creature.getRNG()
            .nextInt(32) && creature.worldObj.isDaytime() && !creature.worldObj.isThundering()) {
            return false;
        } else {
            int l = creature.worldObj.getSavedLightValue(EnumSkyBlock.Block, i, j, k);
            return l <= creature.getRNG()
                .nextInt(8);
        }
    }
