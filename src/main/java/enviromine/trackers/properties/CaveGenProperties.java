...
public class CaveGenProperties implements PropertyBase {
    ...
    public CaveGenProperties(String id, int oreMeta, String source, int srcMeta, int veins, int size, int minY,
        int maxY) {
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        String nID = config.get(category, CGPNames[0], "minecraft:stone")
            .getString();
        int nOreM = MathHelper.clamp_int(
            config.get(category, CGPNames[1], 0)
                .getInt(),
            0,
            15);
        String nSource = config.get(category, CGPNames[2], "minecraft:stone")
            .getString();
        int nSrcM = config.get(category, CGPNames[3], 0)
            .getInt();
        int nVeins = config.get(category, CGPNames[4], 4)
            .getInt();
        int nSize = config.get(category, CGPNames[5], 4)
            .getInt();
        int nMinY = config.get(category, CGPNames[6], 10)
            .getInt();
        int nMaxY = config.get(category, CGPNames[7], 246)
            .getInt();
    ...
    public void customLoad() {
        ...
        try {
            ...
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                Level.ERROR,
                "Failed to load custom configuration for " + this.getClass()
                    .getSimpleName(),
                e);
            return;
        }
