...
public class CaveSpawnProperties implements PropertyBase
{
	...
	public void GenDefaults()
	{
		...
		if(!file.exists())
		{
			...
			{
				if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.LOW.getLevel()) EnviroMine.logger.log(Level.ERROR, "Failed to create file for Cave Ores", e);
				return;
			}
		}
	...
	public void generateEmpty(Configuration config, Object obj)
	{
		...
		{
			if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.ALL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Tried to register config with non entity object!", new Exception());
			return;
		}
	...
	public void customLoad()
	{
		...
		{
			if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Failed to load custom configuration for " + this.getClass().getSimpleName(), e);
			return;
		} 
