...
public class CaveSpawnProperties implements PropertyBase {
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        int nID = config.get(category, CGPNames[0], 0)
            .getInt(0);
        int nWeight = config.get(category, CGPNames[1], 100)
            .getInt(100);
        int nMin = config.get(category, CGPNames[2], 4)
            .getInt(4);
        int nMax = config.get(category, CGPNames[3], 4)
            .getInt(4);
