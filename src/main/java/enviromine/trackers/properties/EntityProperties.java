...
public class EntityProperties implements SerialisableProperty, PropertyBase {
    ...
    private int getID(Entity entity) {
        ...
        if (EntityList.getEntityID(entity) > 0) {
            ...
            .lookupModSpawn(entity.getClass(), false) != null) {
                entityid = EntityRegistry.instance()
    ...
    public EntityProperties(int id, boolean track, boolean dehydration, boolean bodyTemp, boolean airQ,
        boolean immuneToFrost, boolean immuneToHeat, float aSanity, float hSanity, float aTemp, float hTemp, float aAir,
        float hAir, float aHyd, float hHyd, String fileName) {
        this.id = id;
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        int id = config.get(category, EPName[0], 0)
            .getInt(0);
        boolean track = config.get(category, EPName[1], true)
            .getBoolean(true);
        boolean dehydration = config.get(category, EPName[2], true)
            .getBoolean(true);
        boolean bodyTemp = config.get(category, EPName[3], true)
            .getBoolean(true);
        boolean airQ = config.get(category, EPName[4], true)
            .getBoolean(true);
        boolean immuneToFrost = config.get(category, EPName[5], false)
            .getBoolean(false);
        boolean immuneToHeat = config.get(category, EPName[6], false)
            .getBoolean(false);
        float aSanity = (float) config.get(category, EPName[7], 0.0D)
            .getDouble(0.0D);
        float hSanity = (float) config.get(category, EPName[8], 0.0D)
            .getDouble(0.0D);
        float aTemp = (float) config.get(category, EPName[9], 36.6D, "Overridden by body temp")
            .getDouble(36.6D);
        float hTemp = (float) config.get(category, EPName[10], 0.0D)
            .getDouble(0.0D);
        float aAir = (float) config.get(category, EPName[11], 0.0D)
            .getDouble(0.0D);
        float hAir = (float) config.get(category, EPName[12], 0.0D)
            .getDouble(0.0D);
        float aHyd = (float) config.get(category, EPName[13], 0.0D)
            .getDouble(0.0D);
        float hHyd = (float) config.get(category, EPName[14], 0.0D)
            .getDouble(0.0D);
        String filename = config.getConfigFile()
            .getName();

        EntityProperties entry = new EntityProperties(
            id,
            track,
            dehydration,
            bodyTemp,
        ...
        if (EM_Settings.livingProperties.containsKey(id) && !EM_ConfigHandler.loadedConfigs.contains(filename)) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                Level.ERROR,
                "CONFIG DUPLICATE: Entity ID " + id
                    + " was already added from "
                    + EM_Settings.livingProperties.get(id).loadedFrom.toUpperCase()
    ...
    public void SaveProperty(Configuration config, String category) {
        config.get(category, EPName[0], id)
            .getInt(id);
        config.get(category, EPName[1], shouldTrack)
            .getBoolean(shouldTrack);
        config.get(category, EPName[2], dehydration)
            .getBoolean(dehydration);
        config.get(category, EPName[3], bodyTemp)
            .getBoolean(bodyTemp);
        config.get(category, EPName[4], airQ)
            .getBoolean(airQ);
        config.get(category, EPName[5], immuneToFrost)
            .getBoolean(immuneToFrost);
        config.get(category, EPName[6], immuneToHeat)
            .getBoolean(immuneToHeat);
        config.get(category, EPName[7], ambSanity)
            .getDouble(ambSanity);
        config.get(category, EPName[8], hitSanity)
            .getDouble(hitSanity);
        config.get(category, EPName[9], ambTemp, "Overridden by body temp")
            .getDouble(ambTemp);
        config.get(category, EPName[10], hitTemp)
            .getDouble(hitTemp);
        config.get(category, EPName[11], ambAir)
            .getDouble(ambAir);
        config.get(category, EPName[12], hitAir)
            .getDouble(hitAir);
        config.get(category, EPName[13], ambHydration)
            .getDouble(ambHydration);
        config.get(category, EPName[14], hitHydration)
            .getDouble(hitHydration);
    }

    @Override
    public void GenDefaults() {
        @SuppressWarnings("unchecked")
        Iterator<Integer> iterator = EntityList.IDtoClassMapping.keySet()
            .iterator();
        ...
        while (iterator.hasNext()) {
            ...
            if (eID == 65) // Bat
            {
                config.get(catName, EPName[0], eID)
                    .getInt(eID);
                config.get(catName, EPName[1], false)
                    .getBoolean(false);
                config.get(catName, EPName[2], false)
                    .getBoolean(false);
                config.get(catName, EPName[3], false)
                    .getBoolean(false);
                config.get(catName, EPName[4], false)
                    .getBoolean(false);
                config.get(catName, EPName[5], true)
                    .getBoolean(true);
                config.get(catName, EPName[6], true)
                    .getBoolean(true);
                config.get(catName, EPName[7], -0.05D)
                    .getDouble(-0.05D);
                config.get(catName, EPName[8], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[9], 37D, "Overridden by body temp")
                    .getDouble(37D);
                config.get(catName, EPName[10], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[11], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[12], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[13], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[14], 0D)
                    .getDouble(0D);
            } else if (eID == 54) // Zombie
            {
                ...
                config.get(catName, EPName[6], true)
                    .getBoolean(true);
                config.get(catName, EPName[7], -0.1D)
                    .getDouble(-0.1D);
                config.get(catName, EPName[8], -1D)
                    .getDouble(-1D);
                config.get(catName, EPName[9], 10D, "Overridden by body temp")
                    .getDouble(10D);
                config.get(catName, EPName[10], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[11], 0D)
            ...
            } else if (eID == 58) // Enderman
            {
                ...
                config.get(catName, EPName[6], true)
                    .getBoolean(true);
                config.get(catName, EPName[7], -0.5D)
                    .getDouble(-0.5D);
                config.get(catName, EPName[8], -5D)
                    .getDouble(-5D);
                config.get(catName, EPName[9], 10D, "Overridden by body temp")
                    .getDouble(10D);
                config.get(catName, EPName[10], 0D)
            ...
            } else if (eID == 61) // Blaze
            {
                ...
                config.get(catName, EPName[6], true)
                    .getBoolean(true);
                config.get(catName, EPName[7], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[8], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[9], 100D, "Overridden by body temp")
                    .getDouble(100D);
                config.get(catName, EPName[10], 0.5D)
                    .getDouble(0.5D);
                config.get(catName, EPName[11], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[12], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[13], -0.1D)
                    .getDouble(-0.1D);
                config.get(catName, EPName[14], -1D)
                    .getDouble(-1D);
            } else if (eID == 97) // Snowman
            {
                ...
                config.get(catName, EPName[8], 0D)
                    .getDouble(0D);
                config.get(catName, EPName[9], -1D, "Overridden by body temp")
                    .getDouble(-1D);
                config.get(catName, EPName[10], -0.5D)
                    .getDouble(-0.5D);
                config.get(catName, EPName[11], 0D)
    ...
    public void generateEmpty(Configuration config, Object obj) {
        if (obj == null || !(obj instanceof Integer)) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.ALL.getLevel()) EnviroMine.logger
                .log(Level.ERROR, "Tried to register config with non EntityLivingBase id!", new Exception());
            return;
        }
        ...
        config.get(category, EPName[0], id)
            .getInt(id);
        config.get(category, EPName[1], false)
            .getBoolean(false);
        config.get(category, EPName[2], false)
            .getBoolean(false);
        config.get(category, EPName[3], false)
            .getBoolean(false);
        config.get(category, EPName[4], false)
            .getBoolean(false);
        config.get(category, EPName[5], true)
            .getBoolean(true);
        config.get(category, EPName[6], true)
            .getBoolean(true);
        config.get(category, EPName[7], 0D)
            .getDouble(0D);
        config.get(category, EPName[8], 0D)
            .getDouble(hitSanity);
        config.get(category, EPName[9], 35D, "Overridden by body temp")
            .getDouble(35D);
        config.get(category, EPName[10], 0D)
            .getDouble(0D);
        config.get(category, EPName[11], 0D)
            .getDouble(0D);
        config.get(category, EPName[12], 0D)
            .getDouble(0D);
        config.get(category, EPName[13], 0D)
            .getDouble(0D);
        config.get(category, EPName[14], 0D)
            .getDouble(0D);
    }
