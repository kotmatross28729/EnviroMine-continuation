...
public class BlockProperties_Netherlicious {
    ...
    public static void registerNetherliciousTorches(Configuration config, String category, String[] BPName, Block block,
        StabilityType defStability) {
        if (block == ModBlocks.SoulTorch || block == ModBlocks.FoxfireTorch || block == ModBlocks.ShadowTorch) {
            ...
            config.get(category, BPName[15], Block.blockRegistry.getNameForObject(ObjectHandler.offTorch))
                .getString();
        } else if (block == ModBlocks.TorchBone || block == ModBlocks.SoulTorchBone
            || block == ModBlocks.FoxfireTorchBone
            || block == ModBlocks.ShadowTorchBone) {
                config.get(category, BPName[0], Block.blockRegistry.getNameForObject(block))
                    .getString();
                config.get(category, BPName[1], -1)
                    .getInt(-1);
                ...
                config
                    .get(
                        category,
                        BPName[15],
                        Block.blockRegistry.getNameForObject(ObjectHandler_Netherlicious.offTorchBone))
                    .getString();
            }
    }
