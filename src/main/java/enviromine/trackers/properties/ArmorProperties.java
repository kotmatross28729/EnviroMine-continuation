...
public class ArmorProperties implements SerialisableProperty, PropertyBase {
    ...
    public ArmorProperties(Item item, String name, float nightTemp, float shadeTemp, float sunTemp, float nightMult,
        float shadeMult, float sunMult, float sanity, float air, boolean allowCamelPack, String filename,
        boolean isTemperatureResistance, boolean isTemperatureSealed) {
        this.item = item;
        this.name = name;
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        String name = config.get(category, APName[0], "")
            .getString();
        float nightTemp = (float) config.get(category, APName[1], 0.00)
            .getDouble(0.00);
        float shadeTemp = (float) config.get(category, APName[2], 0.00)
            .getDouble(0.00);
        float sunTemp = (float) config.get(category, APName[3], 0.00)
            .getDouble(0.00);
        float nightMult = (float) config.get(category, APName[4], 1.00)
            .getDouble(1.00);
        float shadeMult = (float) config.get(category, APName[5], 1.00)
            .getDouble(1.00);
        float sunMult = (float) config.get(category, APName[6], 1.00)
            .getDouble(1.00);
        float sanity = (float) config.get(category, APName[7], 0.00)
            .getDouble(0.00);
        float air = (float) config.get(category, APName[8], 0.00)
            .getDouble(0.00);
        ...
        if (item instanceof ItemArmor && ((ItemArmor) item).armorType == 1) {
            allowCamelPack = config.get(category, APName[9], true)
                .getBoolean(true);
        }
        boolean isTemperatureResistance = config.get(category, APName[10], false)
            .getBoolean(false);
        boolean isTemperatureSealed = config.get(category, APName[11], false)
            .getBoolean(false);

        ArmorProperties entry = new ArmorProperties(
            (Item) item,
            name,
            nightTemp,
            shadeTemp,
        ...
        if (EM_Settings.armorProperties.containsKey(name) && !EM_ConfigHandler.loadedConfigs.contains(filename)) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                Level.ERROR,
                "CONFIG DUPLICATE: Armor - " + name.toUpperCase()
                    + " was already added from "
                    + EM_Settings.armorProperties.get(name).loadedFrom.toUpperCase()
    ...
    public void SaveProperty(Configuration config, String category) {
        config.get(category, APName[0], name)
            .getString();
        config.get(category, APName[1], nightTemp)
            .getDouble(nightTemp);
        config.get(category, APName[2], shadeTemp)
            .getDouble(shadeTemp);
        config.get(category, APName[3], sunTemp)
            .getDouble(sunTemp);
        config.get(category, APName[4], nightMult)
            .getDouble(nightMult);
        config.get(category, APName[5], shadeMult)
            .getDouble(shadeMult);
        config.get(category, APName[6], sunMult)
            .getDouble(sunMult);
        config.get(category, APName[7], sanity)
            .getDouble(sanity);
        config.get(category, APName[8], air)
            .getDouble(air);
        config.get(category, APName[10], isTemperatureResistance)
            .getBoolean(isTemperatureResistance);
        config.get(category, APName[11], isTemperatureSealed)
            .getBoolean(isTemperatureSealed);
    }
    ...
    public void GenDefaults() {
        ...
        while (itemList.hasNext()) {
            ...
            String[] regName = Item.itemRegistry.getNameForObject(armor)
                .split(":");

            if (regName.length <= 0) {
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                    Level.ERROR,
                    "Failed to get correctly formatted object name for " + armor.getUnlocalizedName());
                continue;
            }
            ...
            if (!armorFile.exists()) {
                try {
                    ...
                    if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.LOW.getLevel()) EnviroMine.logger
                        .log(Level.ERROR, "Failed to create file for " + armor.getUnlocalizedName(), e);
                    continue;
                }
            }
            ...
            String catName = this.categoryName() + "."
                + EnviroUtils.replaceULN(armor.getUnlocalizedName() + "_" + regName[1]);
            ...
            if (armor == Items.diamond_helmet || armor == Items.diamond_chestplate
                || armor == Items.diamond_leggings
                || armor == Items.diamond_boots
                || (armor.getArmorMaterial() == ArmorMaterial.DIAMOND && EM_Settings.genConfigs)) {
                ...
                config.get(catName, APName[8], 0.0D)
                    .getDouble(0.0D);
                config.get(catName, APName[10], false)
                    .getBoolean(false);
                config.get(catName, APName[11], false)
                    .getBoolean(false);

                if (armor.armorType == 1) {
                    config.get(catName, APName[9], true)
                        .getBoolean(true);
                }
            } else if (armor == Items.iron_helmet || armor == Items.iron_chestplate
                || armor == Items.iron_leggings
                || armor == Items.iron_boots
                || (armor.getArmorMaterial() == ArmorMaterial.IRON && EM_Settings.genConfigs)) {
                    config.get(catName, APName[0], Item.itemRegistry.getNameForObject(armor))
                        .getString();
                    config.get(catName, APName[1], -1.0D)
                        .getDouble(-1.0D);
                    config.get(catName, APName[2], 0.0D)
                        .getDouble(0.0D);
                    config.get(catName, APName[3], 2.0D)
                        .getDouble(2.0D);
                    config.get(catName, APName[4], 1.0D)
                        .getDouble(1.0D);
                    config.get(catName, APName[5], 1.0D)
                        .getDouble(1.0D);
                    config.get(catName, APName[6], 1.1D)
                        .getDouble(1.1D);
                    config.get(catName, APName[7], 0.0D)
                        .getDouble(0.0D);
                    config.get(catName, APName[8], 0.0D)
                ...
                } else if (armor == Items.golden_helmet || armor == Items.golden_chestplate
                    || armor == Items.golden_leggings
                    || armor == Items.golden_boots
                    || (armor.getArmorMaterial() == ArmorMaterial.GOLD && EM_Settings.genConfigs)) {
                        config.get(catName, APName[0], Item.itemRegistry.getNameForObject(armor))
                            .getString();
                        config.get(catName, APName[1], 0.0D)
                            .getDouble(0.0D);
                        config.get(catName, APName[2], 0.0D)
                            .getDouble(0.0D);
                        config.get(catName, APName[3], 2.5D)
                            .getDouble(2.5D);
                        config.get(catName, APName[4], 1.0D)
                            .getDouble(1.0D);
                        config.get(catName, APName[5], 1.0D)
                            .getDouble(1.0D);
                        config.get(catName, APName[6], 1.2D)
                            .getDouble(1.2D);
                        config.get(catName, APName[7], 0.0D)
                            .getDouble(0.0D);
                        config.get(catName, APName[8], 0.0D)
                    ...
                    } else if (armor == Items.leather_helmet || armor == Items.leather_chestplate
                        || armor == Items.leather_leggings
                        || armor == Items.leather_boots
                        || (armor.getArmorMaterial() == ArmorMaterial.CLOTH && EM_Settings.genConfigs)) {
                            config.get(catName, APName[0], Item.itemRegistry.getNameForObject(armor))
                                .getString();
                            config.get(catName, APName[1], 1.0D)
                                .getDouble(1.0D);
                            config.get(catName, APName[2], 1.0D)
                                .getDouble(1.0D);
                            config.get(catName, APName[3], 1.0D)
                                .getDouble(1.0D);
                            config.get(catName, APName[4], 1.0D)
                                .getDouble(1.0D);
                            config.get(catName, APName[5], 1.0D)
                                .getDouble(1.0D);
                            config.get(catName, APName[6], 1.0D)
                                .getDouble(1.0D);
                            config.get(catName, APName[7], 0.0D)
                                .getDouble(0.0D);
                            config.get(catName, APName[8], 0.0D)
    ...
    public void generateEmpty(Configuration config, Object obj) {
        ...
        if (regName.length <= 0) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger
                .log(Level.ERROR, "Failed to get correctly formatted object name for " + armor.getUnlocalizedName());
            return;
        }
