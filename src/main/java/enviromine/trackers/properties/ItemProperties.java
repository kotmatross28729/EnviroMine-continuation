...
public class ItemProperties implements SerialisableProperty, PropertyBase {
    ...
    public boolean hasProperty(ItemStack stack) {
        return EM_Settings.itemProperties
            .containsKey("" + Item.itemRegistry.getNameForObject(stack.getItem()) + "," + stack.getItemDamage())
            || EM_Settings.itemProperties.containsKey("" + Item.itemRegistry.getNameForObject(stack.getItem()));
    }
    ...
    public ItemProperties getProperty(ItemStack stack) {
        ...
        if (EM_Settings.itemProperties
            ...
            itemProps = EM_Settings.itemProperties
                .get("" + Item.itemRegistry.getNameForObject(stack.getItem()) + "," + stack.getItemDamage());
        } else {
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        String name = config.get(category, IPName[0], "")
            .getString();
        int meta = config.get(category, IPName[1], 0)
            .getInt(0);
        boolean enableTemp = config.get(category, IPName[2], false)
            .getBoolean(false);
        float ambTemp = (float) config.get(category, IPName[3], 0D)
            .getDouble(0D);
        float ambAir = (float) config.get(category, IPName[4], 0D)
            .getDouble(0D);
        float ambSanity = (float) config.get(category, IPName[5], 0D)
            .getDouble(0D);
        float effTemp = (float) config.get(category, IPName[6], 0D)
            .getDouble(0D);
        float effAir = (float) config.get(category, IPName[7], 0D)
            .getDouble(0D);
        float effSanity = (float) config.get(category, IPName[8], 0D)
            .getDouble(0D);
        ...
        String camelReturnItem = config.get(category, IPName[12], "")
            .getString();
        int camelReturnMeta = config.get(category, IPName[13], 0)
            .getInt(0);
        String filename = config.getConfigFile()
            .getName();

        ItemProperties entry = new ItemProperties(
            name,
            meta,
            enableTemp,
            ambTemp,
        ...
        if (meta < 0) {
            ...
            if (EM_Settings.itemProperties.containsKey("" + name)
                ...
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                    Level.ERROR,
                    "CONFIG DUPLICATE: Items - " + name.toUpperCase()
                        + " was already added from "
                        + EM_Settings.itemProperties.get(name).loadedFrom.toUpperCase()
            ...
            // If item already exist and current file hasn't completely been loaded do this
            if (EM_Settings.itemProperties.containsKey("" + name + "," + meta)
                && !EM_ConfigHandler.loadedConfigs.contains(filename)) {
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                    Level.ERROR,
                    "CONFIG DUPLICATE: Items - " + name.toUpperCase()
    ...
    public void SaveProperty(Configuration config, String category) {
        config.get(category, IPName[0], name)
            .getString();
        config.get(category, IPName[1], meta)
            .getInt(meta);
        config.get(category, IPName[2], enableTemp)
            .getBoolean(enableTemp);
        config.get(category, IPName[3], ambTemp)
            .getDouble(ambTemp);
        config.get(category, IPName[4], ambAir)
            .getDouble(ambAir);
        config.get(category, IPName[5], ambSanity)
            .getDouble(ambSanity);
        config.get(category, IPName[6], effTemp)
            .getDouble(effTemp);
        config.get(category, IPName[7], effAir)
            .getDouble(effAir);
        config.get(category, IPName[8], effSanity)
            .getDouble(effSanity);
        config.get(category, IPName[9], effHydration)
            .getDouble(effHydration);
        config.get(category, IPName[10], effTempCap)
            .getDouble(effTempCap);
        config.get(category, IPName[11], camelFill)
            .getInt(camelFill);
        config.get(category, IPName[12], fillReturnItem)
            .getString();
        config.get(category, IPName[13], fillReturnMeta)
            .getInt(fillReturnMeta);
    }
        ...
        while (iterator.hasNext()) {
            ...
            if (regName.length <= 0) {
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger
                    .log(Level.ERROR, "Failed to get correctly formatted object name for " + item.getUnlocalizedName());
                continue;
            }
            ...
            if (item == Items.glass_bottle) {
                ...
                config.get(category, IPName[5], 0D)
                    .getDouble(0D);
                config.get(category, IPName[6], -0.025D)
                    .getDouble(-0.025D);
                config.get(category, IPName[7], 0D)
                    .getDouble(0D);
                config.get(category, IPName[8], 0D)
                ...
                config.get(category, IPName[2], true)
                    .getBoolean(true);
                config.get(category, IPName[3], -0.1D)
                    .getDouble(-0.1D);
                config.get(category, IPName[4], 0D)
                    .getDouble(0D);
                config.get(category, IPName[5], 0D)
                ...
                config.get(category, IPName[2], true)
                    .getBoolean(true);
                config.get(category, IPName[3], -0.5D)
                    .getDouble(-0.5D);
                config.get(category, IPName[4], 0D)
                    .getDouble(0D);
                config.get(category, IPName[5], 0D)
