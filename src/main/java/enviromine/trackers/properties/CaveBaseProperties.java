...
public class CaveBaseProperties implements PropertyBase {
    ...
    public void GenDefaults() {
        ...
        config.get(catName, CBName[0], -2)
            .getInt(-2);
        config.get(catName, CBName[1], true)
            .getBoolean(true);
        config.get(catName, CBName[2], false, "Makes the dimension more volcanic")
            .getBoolean(false);
        config.get(catName, CBName[3], 30)
            .getInt(30);
        config.get(catName, CBName[4], 7)
            .getInt(7);
        config.get(catName, CBName[5], 8)
            .getInt(8);
        config.get(catName, CBName[6], 23)
            .getInt(23);
        config.get(catName, CBName[7], false)
            .getBoolean(false);
        config.get(catName, CBName[8], 10)
            .getInt(10);
        config.getInt(CBName[9], catName, 32, 0, 255, "Height at which water/lava generates");
        config.get(catName, CBName[10], true)
            .getBoolean(true);
        config.get(catName, CBName[11], false)
            .getBoolean(false);
        config.get(catName, CBName[12], false)
            .getBoolean(false);
        config
            .get(
                catName,
                CBName[13],
                "minecraft:stone",
                "The main block used to populate the Cave dimension. Use the form mod:blockname:meta or mod:blockname")
            .getString()
            .trim();
        config
            .get(
                ...
                CBName[14],
                "minecraft:gravel",
                "A block used to populate the Cave dimension. Use the form mod:blockname:meta or mod:blockname")
            .getString()
            .trim();
        config
            .get(
    ...
    public void customLoad() {
        ...
        EM_Settings.caveDimID = config.get(catName, CBName[0], -2)
            .getInt(-2);
        EM_Settings.caveOreEvent = config.get(catName, CBName[1], true)
            .getBoolean(true);
        EM_Settings.caveLava = config.get(catName, CBName[2], false, "Makes the dimension more volcanic")
            .getBoolean(false);
        EM_Settings.caveRavineRarity = config.get(catName, CBName[3], 30)
            .getInt(30);
        EM_Settings.caveTunnelRarity = config.get(catName, CBName[4], 7)
            .getInt(7);
        EM_Settings.caveDungeons = config.get(catName, CBName[5], 8)
            .getInt(8);
        EM_Settings.caveBiomeID = config.get(catName, CBName[6], 23)
            .getInt(23);
        EM_Settings.disableCaves = config.get(catName, CBName[7], false)
            .getBoolean(false);
        EM_Settings.limitElevatorY = config.get(catName, CBName[8], 10)
            .getInt(10);
        EM_Settings.caveLiquidY = config.getInt(CBName[9], catName, 32, 0, 255, "Height at which water/lava generates");
        EM_Settings.caveFlood = config.get(catName, CBName[10], true)
            .getBoolean(true);
        EM_Settings.caveRespawn = config.get(catName, CBName[11], false)
            .getBoolean(false);
        EM_Settings.enforceWeights = config.get(
            catName,
            CBName[12],
            false,
            "If set to true EnviroMine will try to strictly enforce the configured spawn weights regardless of spawn method. Has some side effects!")
            .getBoolean(false);

        // Load and assign Cave dimension blocks
        String caveStoneRaw = config
            .get(
                catName,
                CBName[13],
                "minecraft:stone",
        ...
        for (int i = 0; i < caveBlockSplitTemp.length; i++) {
        ...
        if (tempBlock != null) {
            ...
            caveGravel_block = Blocks.gravel;
            caveGravel_meta = 0;
        } // Revert to default
