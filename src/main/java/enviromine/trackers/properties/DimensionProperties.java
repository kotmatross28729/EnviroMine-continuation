...
public class DimensionProperties implements SerialisableProperty, PropertyBase {
    ...
    public DimensionProperties(int id, boolean override, boolean trackSanity, boolean darkAffectSanity,
        float sanityMulti, boolean trackAirQuality, float airMulti, boolean trackHydration, float hydrationMulti,
        boolean trackTemp, float tempMulti, boolean dayNightTemp, boolean weatherAffectsTemp, boolean mineshaftGen,
        int sealevel, int mineDepth, float tempRate, float hydrationRate, float sanityRate, float airRate,
        boolean physics, String fileName) {
        this.id = id;
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        int id = config.get(category, DMName[0], 0)
            .getInt(0);
        boolean override = config.get(category, DMName[1], false)
            .getBoolean(false);
        boolean trackSanity = config.get(category, DMName[2], true)
            .getBoolean(true);
        boolean darkAffectSanity = config.get(category, DMName[3], true)
            .getBoolean(true);
        float sanityMulti = (float) config.get(category, DMName[4], 1.0D)
            .getDouble(1.0D);
        boolean trackAirQuality = config.get(category, DMName[5], true)
            .getBoolean(true);
        float airMulti = (float) config.get(category, DMName[6], 1.0D)
            .getDouble(1.0D);
        boolean trackHydration = config.get(category, DMName[7], true)
            .getBoolean(true);
        float hydrationMulti = (float) config.get(category, DMName[8], 1.0D)
            .getDouble(1.0D);
        boolean trackTemp = config.get(category, DMName[9], true)
            .getBoolean(true);
        float tempMulti = (float) config.get(category, DMName[10], 1.0D)
            .getDouble(1.0D);
        boolean dayNightTemp = config.get(category, DMName[11], true)
            .getBoolean(true);
        boolean weatherAffectsTemp = config.get(category, DMName[12], true)
            .getBoolean(true);
        boolean mineshaftGen = config.get(category, DMName[13], true)
            .getBoolean(true);
        int sealevel = config.get(category, DMName[14], 65)
            .getInt(65);
        int mineDepth = config
            .getInt(DMName[15], category, 12, -255, 255, "Use a negitive value to skip shaft entrance checks");
        float tempRate = (float) config.get(category, DMName[16], 0.0D)
            .getDouble(0.0D);
        float hydrationRate = (float) config.get(category, DMName[17], 0.0D)
            .getDouble(0.0D);
        float sanityRate = (float) config.get(category, DMName[18], 0.0D)
            .getDouble(0.0D);
        float airRate = (float) config.get(category, DMName[19], 0.0D)
            .getDouble(0.0D);
        boolean physics = config.get(category, DMName[20], true)
            .getBoolean(true);
        String filename = config.getConfigFile()
            .getName();

        DimensionProperties entry = new DimensionProperties(
            id,
            override,
            trackSanity,
            darkAffectSanity,
        ...
        if (EM_Settings.dimensionProperties.containsKey(id) && !EM_ConfigHandler.loadedConfigs.contains(filename)) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) {
                EnviroMine.logger.log(
                    Level.ERROR,
                    "CONFIG DUPLICATE: Dimension ID " + id
                        + " was already added from "
                        + EM_Settings.dimensionProperties.get(id).loadedFrom.toUpperCase()
    ...
    public void SaveProperty(Configuration config, String category) {
        config.get(category, DMName[0], id)
            .getInt(id);
        config.get(category, DMName[1], override)
            .getBoolean(override);
        config.get(category, DMName[2], trackSanity)
            .getBoolean(trackSanity);
        config.get(category, DMName[3], darkAffectSanity)
            .getBoolean(darkAffectSanity);
        config.get(category, DMName[4], sanityMulti)
            .getDouble(sanityMulti);
        config.get(category, DMName[5], trackAirQuality)
            .getBoolean(trackAirQuality);
        config.get(category, DMName[6], airMulti)
            .getDouble(airMulti);
        config.get(category, DMName[7], trackHydration)
            .getBoolean(trackHydration);
        config.get(category, DMName[8], hydrationMulti)
            .getDouble(hydrationMulti);
        config.get(category, DMName[9], trackTemp)
            .getBoolean(trackTemp);
        config.get(category, DMName[10], tempMulti)
            .getDouble(tempMulti);
        config.get(category, DMName[11], dayNightTemp)
            .getBoolean(dayNightTemp);
        config.get(category, DMName[12], weatherAffectsTemp)
            .getBoolean(weatherAffectsTemp);
        config.get(category, DMName[13], mineshaftGen)
            .getBoolean(mineshaftGen);
        config.get(category, DMName[14], sealevel)
            .getInt(sealevel);
        config.getInt(DMName[15], category, mineDepth, -255, 255, "Use a negitive value to skip shaft entrance checks");
        config.get(category, DMName[16], 0.0D)
            .getDouble(0.0D);
        config.get(category, DMName[17], 0.0D)
            .getDouble(0.0D);
        config.get(category, DMName[18], 0.0D)
            .getDouble(0.0D);
        config.get(category, DMName[19], 0.0D)
            .getDouble(0.0D);
        config.get(category, DMName[20], physics)
            .getBoolean(physics);
    }
    ...
    public void GenDefaults() {
        ...
        for (int i = 0; i < dimIDs.length; i++) {
            ...
            if (!file.exists()) {
                try {
                    ...
                    if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.LOW.getLevel()) EnviroMine.logger.log(
                        Level.ERROR,
                        "Failed to create file for dimension '" + dimension.getDimensionName() + "'",
                        e);
                    continue;
                }
            }
            ...
            if (dimension.dimensionId == EM_Settings.caveDimID) {
                config.get(catName, DMName[0], dimension.dimensionId)
                    .getInt(dimension.dimensionId);
                config.get(catName, DMName[1], true)
                    .getBoolean(true);
                config.get(catName, DMName[2], true)
                    .getBoolean(true);
                config.get(catName, DMName[3], true)
                    .getBoolean(true);
                config.get(catName, DMName[4], 1.0D)
                    .getDouble(1.0D);
                config.get(catName, DMName[5], true)
                    .getBoolean(true);
                config.get(catName, DMName[6], 1.0D)
                    .getDouble(1.0D);
                config.get(catName, DMName[7], true)
                    .getBoolean(true);
                config.get(catName, DMName[8], 1.0D)
                    .getDouble(1.0D);
                config.get(catName, DMName[9], true)
                    .getBoolean(true);
                config.get(catName, DMName[10], 1.0D)
                    .getDouble(1.0D);
                config.get(catName, DMName[11], true)
                    .getBoolean(true);
                config.get(catName, DMName[12], true)
                    .getBoolean(true);
                config.get(catName, DMName[13], true)
                    .getBoolean(true);
                config.get(catName, DMName[14], 340)
                    .getInt(340);
                config
                    .getInt(DMName[15], catName, -192, -255, 255, "Use a negitive value to skip shaft entrance checks");
                config.get(catName, DMName[16], 0.0D)
                    .getDouble(0.0D);
                config.get(catName, DMName[17], 0.0D)
                    .getDouble(0.0D);
                config.get(catName, DMName[18], 0.0D)
                    .getDouble(0.0D);
                config.get(catName, DMName[19], 0.0D)
                    .getDouble(0.0D);
                config.get(catName, DMName[20], true)
                    .getBoolean(true);
                ...
                config.get(catName, DMName[12], true)
                    .getBoolean(true);
                config.get(catName, DMName[13], false)
                    .getBoolean(false);
                config.get(catName, DMName[14], 340)
                    .getInt(340);
                config
    ...
    public void generateEmpty(Configuration config, Object obj) {
        if (obj == null || !(obj instanceof WorldProvider)) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.ALL.getLevel()) EnviroMine.logger
                .log(Level.ERROR, "Tried to register config with non WorldProvider object!", new Exception());
            return;
        }
        ...
        config.get(catName, DMName[10], 1.0D)
            .getDouble(1.0D);
        config.get(catName, DMName[11], !dimension.hasNoSky)
            .getBoolean(!dimension.hasNoSky);
        config.get(catName, DMName[12], !dimension.hasNoSky)
            .getBoolean(!dimension.hasNoSky);
        config.get(catName, DMName[13], !dimension.hasNoSky)
            .getBoolean(!dimension.hasNoSky);
        config.get(catName, DMName[14], dimension.hasNoSky ? 340 : 64)
            .getInt(dimension.hasNoSky ? 340 : 64);
