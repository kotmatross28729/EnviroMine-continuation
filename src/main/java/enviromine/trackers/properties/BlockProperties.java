...
import enviromine.trackers.properties.compat.BlockProperties_TC;
...
public class BlockProperties implements SerialisableProperty, PropertyBase {
    ...
    public boolean hasProperty(Block block, int meta) {
        return EM_Settings.blockProperties.containsKey("" + Block.blockRegistry.getNameForObject(block) + "," + meta)
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        String name = config.get(category, BPName[0], "")
            .getString();
        int metaData = config.get(category, BPName[1], 0)
            .getInt(0);
        String dropName = config.get(category, BPName[2], "")
            .getString();
        int dropMeta = config.get(category, BPName[3], 0)
            .getInt(0);
        int dropNum = config.get(category, BPName[4], -1)
            .getInt(-1);
        boolean enableTemp = config.get(category, BPName[5], false)
            .getBoolean(false);
        float temperature = (float) config.get(category, BPName[6], 0.00)
            .getDouble(0.00);
        float airQuality = (float) config.get(category, BPName[7], 0.00)
            .getDouble(0.00);
        float sanity = (float) config.get(category, BPName[8], 0.00)
            .getDouble(0.00);
        String stability = config.get(category, BPName[9], EM_Settings.defaultStability)
            .getString();
        boolean slides = config.get(category, BPName[10], false)
            .getBoolean(false);
        boolean wetSlides = config.get(category, BPName[11], false)
            .getBoolean(false);

        String filename = config.getConfigFile()
            .getName();

        boolean goOut = config.get(category, BPName[12], false)
            .getBoolean(false);
        boolean goOutRain = config.get(category, BPName[13], false)
            .getBoolean(false);
        int goOutChance = config.get(category, BPName[14], 10000)
            .getInt(10000);
        ...
        if (metaData < 0) {
            ...
            // If item already exist and current file hasn't completely been loaded do this
            if (EM_Settings.blockProperties.containsKey("" + name + "," + metaData)
                && !EM_ConfigHandler.loadedConfigs.contains(filename)) {
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                    Level.ERROR,
                    "CONFIG DUPLICATE: Block - " + name.toUpperCase()
    ...
    public void SaveProperty(Configuration config, String category) {
        config.get(category, BPName[0], name)
            .getString();
        config.get(category, BPName[1], meta)
            .getInt(0);
        config.get(category, BPName[2], dropName)
            .getString();
        config.get(category, BPName[3], dropMeta)
            .getInt(0);
        config.get(category, BPName[4], dropNum)
            .getInt(-1);
        config.get(category, BPName[5], enableTemp)
            .getBoolean(false);
        config.get(category, BPName[6], temp)
            .getDouble(0.00);
        config.get(category, BPName[7], air)
            .getDouble(0.00);
        config.get(category, BPName[8], sanity)
            .getDouble(0.00);
        config.get(category, BPName[9], stability)
            .getString();
        config.get(category, BPName[10], slides)
            .getBoolean(false);
    ...
    public void GenDefaults() {
        ...
        while (iterator.hasNext()) {
            ...
            if (regName.length <= 0) {
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                    Level.ERROR,
                    "Failed to get correctly formatted object name for " + block.getUnlocalizedName());
                continue;
            }
            ...
            String category = this.categoryName() + "."
                + EnviroUtils.replaceULN(block.getUnlocalizedName() + "_" + regName[1]);
            ...
            if (block == Blocks.lava || block == Blocks.flowing_lava
                ...
                || (EM_Settings.genConfigs && block.getMaterial() == Material.lava)) {
                ...
                config.get(category, BPName[5], true)
                    .getBoolean(true);
                config.get(category, BPName[6], 200.0D)
                    .getDouble(200.0D);
                config.get(category, BPName[7], -1D)
                    .getDouble(-1D);
    ...
    public void generateEmpty(Configuration config, Object obj) {
        ...
        if (regName.length <= 0) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                Level.ERROR,
                "Failed to get correctly formatted object name for " + block.getUnlocalizedName() + "_" + regName[1]);
            return;
        }
        ...
        config
            .get(
                category,
                BPName[2],
                block == Blocks.stone ? Block.blockRegistry.getNameForObject(Blocks.cobblestone)
                    : (block == Blocks.grass ? Block.blockRegistry.getNameForObject(Blocks.dirt) : ""))
            .getString();
        config.get(category, BPName[3], -1)
            .getInt(0);
