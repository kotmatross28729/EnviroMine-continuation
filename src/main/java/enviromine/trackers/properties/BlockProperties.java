...
public class BlockProperties implements SerialisableProperty, PropertyBase
{
	...
	public void LoadProperty(Configuration config, String category)
	{
		...
		{
			if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.WARN, "Stability type '" + stability + "' not found.");
			minFall = 99;
			maxFall = 99;
			supportDist = 9;
			hasPhys = false;
		...
		if(metaData < 0)
		{
			...
			{
				if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "CONFIG DUPLICATE: Block - "+name.toUpperCase() +" was already added from "+ EM_Settings.blockProperties.get(name).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
			}
		...
		} else
		{
			...
			{
				if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "CONFIG DUPLICATE: Block - "+name.toUpperCase() +" - Meta:"+ meta +"  was already added from "+ EM_Settings.blockProperties.get(name).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
			}
	...
	public void GenDefaults()
	{
		...
		while(iterator.hasNext())
		{
			...
			if(!blockFile.exists())
			{
				...
				{
					if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.LOW.getLevel()) EnviroMine.logger.log(Level.ERROR, "Failed to create file for " + regName[1], e);
					continue;
				}
			}
