...
public class BlockElevator extends Block implements ITileEntityProvider {
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int par6, float par7,
         ...
         * {
         * if(meta1 == 2 && (world.getBlock(i, j - 1, k) != ObjectHandler.elevator || world.getBlockMetadata(i, j - 1,
         * k) != 3))
         * {
         * player.addChatMessage(new ChatComponentText("Can't recall an elevator. Incomplete frame"));
         * } else if(meta1 == 3 && (world.getBlock(i, j + 1, k) != ObjectHandler.elevator || world.getBlockMetadata(i, j
         * + 1, k) != 2))
         * {
         * player.addChatMessage(new ChatComponentText("Can't recall an elevator. Incomplete frame"));
         * } else
        ...
        if (!(meta == 0 && world.getBlock(i, j - 1, k) == ObjectHandler.elevator
            && world.getBlockMetadata(i, j - 1, k) % 2 == 1)
            && !(meta == 1 && world.getBlock(i, j + 1, k) == ObjectHandler.elevator
                && world.getBlockMetadata(i, j + 1, k) % 2 == 0)) {
        ...
        if (player.dimension == EM_Settings.caveDimID) {
            ...
            if (player.getEntityData()
                .hasKey("EM_CAVE_DIST")) {
                if (player.getEntityData()
                    .getIntArray("EM_CAVE_DIST")[3] >= 1000) {
                    player.addStat(EnviroAchievements.intoTheDarkness, 1);
                }

                player.getEntityData()
                    .removeTag("EM_CAVE_DIST");
            }
            ...
            playerMP.mcServer.getConfigurationManager()
                .transferPlayerToDimension(
                    playerMP,
                    0,
                    TeleportHandler.GetInstance(playerMP.mcServer.worldServerForDimension(0)));
            ...
            player.getEntityData()
                .setIntArray("EM_CAVE_DIST", new int[] { i, j, k, 0 });
            playerMP.mcServer.getConfigurationManager()
                .transferPlayerToDimension(
                    playerMP,
                    EM_Settings.caveDimID,
                    TeleportHandler.GetInstance(playerMP.mcServer.worldServerForDimension(EM_Settings.caveDimID)));
            // world.setBlockToAir(i, j, k);
            /*
             * if(meta == 0)
             * {
