...
public class BlockFireTorch extends BlockTorch {
    ...
    public Item getItemDropped(int p_149650_1_, Random p_149650_2_, int p_149650_3_) {
        return Item.getItemFromBlock(
            (EM_Settings.torchesBurn || EM_Settings.torchesGoOut) ? ObjectHandler.fireTorch : Blocks.torch);
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if ((!EM_Settings.torchesBurn && !EM_Settings.torchesGoOut) || !EM_Settings.oldTorchLogic) {
            world.setBlock(x, y, z, Blocks.torch, world.getBlockMetadata(x, y, z), 3);
            return;
        }

        if ((world.rand.nextInt(10000) == 0 || (world.isRaining() && world.canBlockSeeTheSky(x, y, z)))
            && EM_Settings.torchesGoOut) {
