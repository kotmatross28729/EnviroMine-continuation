...
public class TileEntityGas extends TileEntity {
    ...
    public void readFromNBT(NBTTagCompound par1NBTTagCompound) {
        ...
        if (!par1NBTTagCompound.hasKey("GasArray")) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.LOW.getLevel()) EnviroMine.logger
                .log(Level.ERROR, "GasTile missing NBT 'GasArray', this should not happen!", new Exception());
            return;
        }
        ...
        if (savedGases.length <= 0) {
            // EnviroMine.logger.log(Level.ERROR, "GasTile loaded " + savedGases.length + " gases, this should not
            // happen!", new Exception());
            return;
        }
        ...
        if (gases.size() <= 0) {
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                Level.ERROR,
                "GasTile loaded " + savedGases.length + " gas units but no gases were actually added!",
                new Exception());
            return;
        }
    }
    ...
    public void updateAmount() {
        ...
        if (this.amount != before) {
            // EnviroMine.logger.log(Level.INFO, "GasTile amount updated: " + this.amount + ", Pos: {" + xCoord + "," +
            // yCoord + "," + zCoord + "}");
            this.markDirty();
        }
    }
    ...
    public void checkNeedsReRender() {
        ...
        for (int i = 0; i < 6; i++) {
            if (((BlockGas) this.getBlockType())
                .shouldSideBeRendered(this.worldObj, this.xCoord, this.yCoord, this.zCoord, i)) {
                shouldRender = true;
            }
        }
    ...
    public void addGas(int id, int addNum) {
        ...
        if (id == 0 && this.hasWorldObj()) {
            if (this.getBlockType() == ObjectHandler.gasBlock) {
                this.burnGases();
                ((BlockGas) this.getBlockType())
                    .swtichIgnitionState(this.getWorldObj(), this.xCoord, this.yCoord, this.zCoord);
            }
        }
    ...
    public boolean burnGases() {
        ...
        for (int i = 0; i < gases.size(); i++) {
            int[] gasArray = gases.get(i);
            int fire = EnviroGasDictionary.gasList[gasArray[0]]
                .getFire(gasArray[1], this.amount >= 10 ? 0 : 10 - this.amount);
    ...
    public boolean spreadGas() {
        ...
        for (int i = gDir.size(); i > 0; i--) {
            ...
            if (rDir[1] == 0 && this.amount <= 1 || !this.worldObj.getChunkProvider()
                .chunkExists((this.xCoord + rDir[0]) / 16, (this.zCoord + rDir[2]) / 16)) {
                gDir.remove(index);
                continue;
            }

            TileEntity tile = this.worldObj
                .getTileEntity(this.xCoord + rDir[0], this.yCoord + rDir[1], this.zCoord + rDir[2]);

            if ((tile != null && tile instanceof TileEntityGas)
                || this.worldObj.getBlock(this.xCoord + rDir[0], this.yCoord + rDir[1], this.zCoord + rDir[2])
                    == Blocks.air) {
    ...
    public boolean offLoadGas(int i, int j, int k, int offLoadNum) {
        ...
        if (tile == null) {
            ...
            if (gasTile.amount + gasMode >= this.amount && this.amount <= 10
                && vDir == 0
                && this.getBlockType() != ObjectHandler.fireGasBlock) {
                return false;
            } else if (vDir != 0 && this.amount <= 10
                && gasTile.amount >= 10
                && this.getBlockType() != ObjectHandler.fireGasBlock) {
                    return false;
                }
            ...
            if (EnviroGasDictionary.gasList[gases.get(0)[0]].density < 0F && vDir == 1
                && (gasTile.amount < 10 || this.amount > 10)) {
                selGas = gases.get(0);
            } else if (EnviroGasDictionary.gasList[gases.get(gases.size() - 1)[0]].density > 0F && vDir == -1
                && (gasTile.amount < 10 || this.amount > 10)) {
    ...
    public boolean doDecay() {
        ...
        if (this.yCoord > 0 && chunk != null) {
            if (this.yCoord >= 256) {
                ...
                skyLight = chunk
                    .getSavedLightValue(EnumSkyBlock.Sky, this.xCoord & 0xf, this.yCoord, this.zCoord & 0xf);
            }
        }
        ...
        for (int i = gases.size() - 1; i >= 0; i--) {
            ...
            if (gasType.normDecay > 0 && gasType.normDecayThresh >= gasArray[1]) {
                ...
                return decayed;
            } else if (this.worldObj.rand.nextInt(100) == 0 && gasType.randDecay > 0
                && gasType.randDecayThresh >= gasArray[1]) {
                    decayed = true;
