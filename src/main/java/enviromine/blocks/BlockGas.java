...
public class BlockGas extends Block implements ITileEntityProvider {
    ...
    public void onBlockAdded(World world, int i, int j, int k) {
        ...
        if (this == ObjectHandler.fireGasBlock) {
            for (int dir = 0; dir < ForgeDirection.VALID_DIRECTIONS.length; dir++) {
                ...
                if ((sBlock.isFlammable(world, xOff, yOff, zOff, ForgeDirection.VALID_DIRECTIONS[dir].getOpposite())
                    && !(sBlock instanceof BlockGas)) || sBlock == Blocks.air) {
    ...
    public boolean shouldSideBeRendered(IBlockAccess blockAccess, int i, int j, int k, int side) {
        ...
        if (blockAccess.getBlock(sideCoord[0], sideCoord[1], sideCoord[2]) == ObjectHandler.gasBlock
            || blockAccess.getBlock(sideCoord[0], sideCoord[1], sideCoord[2]) == ObjectHandler.fireGasBlock) {
            ...
            if (side == 0 && yMin != 0.0F) {
                ...
                return !blockAccess.getBlock(sideCoord[0], sideCoord[1], sideCoord[2])
                    .isOpaqueCube();
            }
        }
    }
    ...
    public boolean isTouchingIgnition(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < dir.size(); i++) {
            ...
            if (ObjectHandler.igniteList.containsKey(block) && (ObjectHandler.igniteList.get(block)
                .isEmpty()
                || ObjectHandler.igniteList.get(block)
                    .contains(meta))) {
                return true;
            } else {
        ...
        if (tile instanceof TileEntityGas) {
            ...
            if (entityLiving instanceof EntityPlayer) {
                ...
                for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
                    ...
                    if (stack != null && stack.getItem() == Item.getItemFromBlock(ObjectHandler.davyLampBlock)
            ...
            if (tile != null && tile instanceof TileEntityGas) {
                ...
                if (gasTile.burnGases()) {
                    if (gasTile.getBlockType() == ObjectHandler.fireGasBlock) {
                        ((BlockGas) gasTile.getBlockType())
                            .swtichIgnitionState(world, x + pos[0], y + pos[1], z + pos[2]);
                        world.playSoundEffect(
                            x,
                            y,
                            z,
