 ...
 * @author Funwayguy
 *
 *         This class is used to batch process the gas entity updates so as to sync up all their movements and allow for
    ...
    public static void update() {
        ...
        if (curTick % Math.max(1, EM_Settings.gasTickRate / 4) == 0) {
            for (int i = 0; i < fireBuffer.size(); i++) {
                ...
                World world = MinecraftServer.getServer()
                    .worldServerForDimension(entry[0]);

                if (world == null
                    || world.getTotalWorldTime() < EM_PhysManager.worldStartTime + EM_Settings.worldDelay) {
                    continue;
                } else if (EM_PhysManager.chunkDelay
                    .containsKey(world.provider.dimensionId + "" + (entry[1] >> 4) + "," + (entry[3] >> 4))) {
                        if (EM_PhysManager.chunkDelay
                            .get(world.provider.dimensionId + "" + (entry[1] >> 4) + "," + (entry[3] >> 4))
                            > world.getTotalWorldTime()) {
                            continue;
                        }
                    }

                if (EM_Settings.gasPassLimit >= 0
                    && getChunkUpdates(entry[1] >> 4, entry[3] >> 4) >= EM_Settings.gasPassLimit / 4) {
                    continue;
                }
        ...
        if (curTick % Math.max(1, EM_Settings.gasTickRate) == 0) {
            for (int i = 0; i < gasBuffer.size(); i++) {
                ...
                if (EM_Settings.gasPassLimit >= 0
                    && getChunkUpdates(entry[1] >> 4, entry[3] >> 4) >= EM_Settings.gasPassLimit) {
                    continue;
                }

                if (world != null && world.getChunkProvider()
                    .chunkExists(entry[1] >> 4, entry[3] >> 4)
                    && world.getChunkFromBlockCoords(entry[1], entry[3]).isChunkLoaded
                    && world.getBlock(entry[1], entry[2], entry[3]) instanceof BlockGas) {
                    Block block = world.getBlock(entry[1], entry[2], entry[3]);
                    world.scheduleBlockUpdateWithPriority(entry[1], entry[2], entry[3], block, 1, 0);
                    incrementUpdates(entry[1] >> 4, entry[3] >> 4);
                }
