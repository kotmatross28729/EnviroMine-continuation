...
public class GasSulfurDioxide extends EnviroGas {
    ...
    public void applyEffects(EntityLivingBase entityLiving, int amplifier) {
        ...
        if (SulfurDioxideGasDebugLogger) {
            ...
            logger.warn(
                "SulfurDioxide: entityLiving.getRNG().nextInt(SulfurDioxidePoisoningChance) == 0?:  "
                    + (entityLiving.getRNG()
                        .nextInt(SulfurDioxidePoisoningChance) == 0));
        }
        if (!hasGasMask && !isCreative
            && amplifier >= SulfurDioxidePoisoningAmplifier
                + ((EM_Settings.witcheryVampireImmunities && entityLiving instanceof EntityPlayer)
                    ...
                    : 0)
            && entityLiving.getRNG()
                .nextInt(SulfurDioxidePoisoningChance) == 0) // 100
        {
            if (amplifier >= SulfurDioxideSeverePoisoningAmplifier
                + ((EM_Settings.witcheryVampireImmunities && entityLiving instanceof EntityPlayer)
                ...
                entityLiving.addPotionEffect(
                    new PotionEffect(
                        Potion.poison.id,
                        SulfurDioxideSeverePoisoningTime,
                        SulfurDioxideSeverePoisoningLevel));
            } else {
                entityLiving.addPotionEffect(
                    new PotionEffect(Potion.poison.id, SulfurDioxidePoisoningTime, SulfurDioxidePoisoningLevel));
            }
        }
    }
}
