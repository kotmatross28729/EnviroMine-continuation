...
public class EnviroArmor extends ItemArmor //implements ITextureProvider, IArmorTextureProvider
{
	...
	public void onUpdate(ItemStack armor, World world, Entity entity, int itemSlot, boolean isSelected)
	{
		if (armor.getItem() == ObjectHandler.camelPack)
		{
			...
			if (!armor.getTagCompound().hasKey(EM_Settings.CAMEL_PACK_FILL_TAG_KEY))
			{
				int meta = armor.getItemDamage() > 0 ? 100 - armor.getItemDamage() : 0;
				armor.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_FILL_TAG_KEY, meta);
				armor.setItemDamage(0);
			}
			if (!armor.getTagCompound().hasKey(EM_Settings.CAMEL_PACK_MAX_TAG_KEY))
			{
				armor.getTagCompound().setInteger(EM_Settings.CAMEL_PACK_MAX_TAG_KEY, EM_Settings.camelPackMax);
			}
			if (!armor.getTagCompound().hasKey(EM_Settings.IS_CAMEL_PACK_TAG_KEY))
			{
				armor.getTagCompound().setBoolean(EM_Settings.IS_CAMEL_PACK_TAG_KEY, true);
			}
		} else if (armor.getItem() == ObjectHandler.gasMask)
		{
			...
			if (!armor.getTagCompound().hasKey(EM_Settings.GAS_MASK_FILL_TAG_KEY))
			{
				armor.getTagCompound().setInteger(EM_Settings.GAS_MASK_FILL_TAG_KEY, EM_Settings.gasMaskMax);
			}
			if (!armor.getTagCompound().hasKey(EM_Settings.GAS_MASK_MAX_TAG_KEY))
			{
				armor.getTagCompound().setInteger(EM_Settings.GAS_MASK_MAX_TAG_KEY, EM_Settings.gasMaskMax);
			}
