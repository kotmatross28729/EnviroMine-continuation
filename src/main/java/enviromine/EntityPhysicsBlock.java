...
public class EntityPhysicsBlock extends EntityFallingBlock implements IEntityAdditionalSpawnData {
    ...
    public EntityPhysicsBlock(World world) {
        ...
        if (EM_Settings.entityFailsafe > 0 && !world.isRemote) {
            ...
            List<EntityPhysicsBlock> entityList = this.worldObj
                .getEntitiesWithinAABB(EntityPhysicsBlock.class, this.boundingBox.expand(8F, 8F, 8F));
            ...
            if (entityList.size() >= 1024) {
                if (EM_Settings.entityFailsafe == 1) {
                    ...
                    if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) {
                        ...
                        EnviroMine.logger
                            .log(Level.ERROR, "Location: " + this.posX + "," + this.posY + "," + this.posZ);
    ...
    public EntityPhysicsBlock(World world, double x, double y, double z, Block block, int meta, boolean update) {
        ...
        EM_PhysManager.usedSlidePositions
            .add("" + MathHelper.floor_double(this.posX) + "," + MathHelper.floor_double(this.posZ));
        ...
        if (this.block == null || this.block == Blocks.air) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.fallTime == 1) {
                    ...
                    List<EntityItem> before = this.worldObj
                        .getEntitiesWithinAABB(EntityItem.class, this.boundingBox.expand(1, 1, 1));
                    this.worldObj.setBlockToAir(i, j, k);
                    List<EntityItem> after = this.worldObj
                        .getEntitiesWithinAABB(EntityItem.class, this.boundingBox.expand(1, 1, 1));

                    for (EntityItem e : before) {
                        after.remove(e);
                    }
                ...
                try {
                    ...
                    if (axisalignedbb != null) {
                        List fallingBlocks = this.worldObj
                            .getEntitiesWithinAABB(EntityPhysicsBlock.class, axisalignedbb);
                ...
                if (earthquake) {
                    for (int jj = j; j >= MathHelper.ceiling_double_int(motionY); j--) {
                        if (jj <= 5) {
                            this.setDead();
                            return;
                        } else if (this.worldObj.getBlock(i, jj, k)
                            .getMaterial() == Material.lava) {
                                ...
                                .getMaterial() == Material.water) {
                                    this.setDead();
                                    return;
                                } else if (this.worldObj.getBlock(i, jj, k) == Blocks.obsidian
                                    || this.worldObj.getBlock(i, jj, k) == Blocks.cobblestone) {
                                        if (this.worldObj.getBlock(i, jj - 1, k)
                                            .getMaterial() == Material.lava) {
                                            this.setDead();
                                            return;
                                        }
                ...
                if (this.onGround) {
                    ...
                    if (this.worldObj.getBlock(i, j, k) != Blocks.piston_extension) {
                        ...
                        if (!this.worldObj
                            .canPlaceEntityOnSide(Blocks.anvil, i, j, k, true, 1, (Entity) null, (ItemStack) null)
                            && !EM_PhysManager.blockNotSolid(this.worldObj, i, j, k, false)) {
                            j += 1;
                        }

                        if ((this.block == Blocks.snow_layer && this.worldObj.getBlock(i, j, k) == Blocks.snow_layer
                            && this.worldObj.getBlockMetadata(i, j, k) < 15)
                            || (!this.isBreakingAnvil2 && this.worldObj
                                .canPlaceEntityOnSide(Blocks.anvil, i, j, k, true, 1, (Entity) null, (ItemStack) null)
                                && !BlockFalling.func_149831_e(this.worldObj, i, j - 1, k)
                                && this.worldObj.setBlock(i, j, k, this.block, this.meta, 3))) {
                            if (this.block == Blocks.snow_layer && this.worldObj.getBlock(i, j, k) == Blocks.snow_layer
                                && this.worldObj.getBlockMetadata(i, j, k) < 15) {
                                if (this.worldObj.getBlockMetadata(i, j, k) >= 14) {
                                    ...
                                    this.worldObj.setBlockMetadataWithNotify(
                                        ...
                                        j,
                                        k,
                                        this.worldObj.getBlockMetadata(i, j, k) + 1,
                                        3);
                                }
                            ...
                            EM_PhysManager
                                .schedulePhysUpdate(this.worldObj, i, j, k, true, earthquake ? "Quake" : "Collapse");
                            ...
                            Block bSurface = null;
                            if (this.worldObj.getBlock(i, j - 1, k) != Blocks.air && this.worldObj.getBlock(i, j - 1, k)
                            ...
                            if (bSurface == null || bSurface != this.block) {
                                this.worldObj.playSoundEffect(
                                    ...
                                    this.block.stepSound.getPitch() * 0.5F);
                            }
                            ...
                            this.entityDropItem(
                                new ItemStack(this.block, 1, this.block.damageDropped(this.meta)),
                                0.0F);
                        }
                    }
                } else
            ...
            if (i > 0) {
                ...
                while (iterator.hasNext()) {
                    ...
                    entity.attackEntityFrom(
                        damagesource,
                        (float) Math.min(MathHelper.floor_float((float) i * this.fallHurtAmount2), this.fallHurtMax2));
                }

                if (this.block == Blocks.anvil
                    && (double) this.rand.nextFloat() < 0.05000000074505806D + (double) i * 0.05D) {
                    int j = this.meta >> 2;
                    int k = this.meta & 3;
                    ++j;
