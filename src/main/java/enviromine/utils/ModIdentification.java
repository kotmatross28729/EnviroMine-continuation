	...
	public static String idFromObject(Object obj)
	{
		...
		try
		{
			...
			if(clazz == null)
			{
				...
				if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "ModID lookup failed for: NULL");
				return "unknown";
			}
		...
		} catch(Exception e)
		{
			...
			else if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.INFO, "ModID lookup failed for: " + (obj instanceof Class? ((Class)obj).getName() : obj.getClass().getName()) + " {" + fullPath + "}");
