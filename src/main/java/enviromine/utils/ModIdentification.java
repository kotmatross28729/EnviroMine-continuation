...
public class ModIdentification {
    ...
    // Now uses full path (as a File object) instead of only the file name. This is to prevent identically named files
    ...
    public static String idFromObject(Object obj) {
        ...
        if (obj instanceof ItemStack) {
            ...
            EntityRegistration er = EntityRegistry.instance()
                .lookupModSpawn((Class<? extends Entity>) clazz, true);
        ...
        // BRUTE FORCE IDENTIFIER: This is where non-standard objects are identified through their mod file location.
        ...
        // Slight changes here so you can throw raw classes at this and still get the correct result as well as
        // converting the string to just a file object and additional error handling
        try {
            ...
            // EnviroMine.logger.log(Level.INFO, "ModID lookup Success for: " + (obj instanceof Class?
            // ((Class)obj).getName() : obj.getClass().getName()) + " {" + fullPath + "}");

            for (File s : modSource_ID.keySet()) {
                if (file.equals(s) || file.getAbsolutePath()
                    .startsWith(s.getAbsolutePath())) {
            ...
            // EnviroMine.logger.log(Level.INFO, "ModID lookup failed for: " + (obj instanceof Class?
            // ((Class)obj).getName() : obj.getClass().getName()) + " {" + fullPath + "}", e);

            if ((obj instanceof Class ? ((Class) obj).getName()
                : obj.getClass()
                    .getName()).toLowerCase()
                        .contains("net.minecraft")) {
                ...
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel())
                    EnviroMine.logger
                        .log(
                            Level.INFO,
                            "ModID lookup failed for: "
                                + (obj instanceof Class ? ((Class) obj).getName()
                                    : obj.getClass()
                                        .getName())
                                + " {"
                                + fullPath
    ...
    private static String OldIdentificationMethod(Class<?> obj) {
        ...
        try {
            objPath = obj.getProtectionDomain()
                .getCodeSource()
                .getLocation()
                .toString();
        } catch (Exception e) {
    ...
    static {
        ...
        for (ModContainer mod : Loader.instance()
            ...
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                Level.INFO,
                "Mapped mod: {" + mod.getSource()
                    .getAbsolutePath() + "," + mod.getName() + "," + mod.getModId() + "}");
        }
    }
}
