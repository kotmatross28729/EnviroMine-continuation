...
public class Earthquake
{
	...
	public static void TickDay(World world)
	{
		if(world.rand.nextInt(2) == 0 && world.playerEntities.size() > 0)
		{
			...
			if(world.getChunkProvider().chunkExists(posX >> 4, posZ >> 4))
			{
				...
				if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.ALL.getLevel()) EnviroMine.logger.log(Level.INFO, "Earthquake at (" + posX + "," + posZ + ")");
			}
		}
	}
	...
	public static void saveQuakes(File file)
	{
		...
		{
			if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Failed to save Earthquakes", e);
			e.printStackTrace();
		}
	}
	...
	public static void loadQuakes(File file)
	{
		...
		} else
		{
			...
			{
				if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(Level.ERROR, "Failed to load Earthquakes", e);
				e.printStackTrace();
			}
		}
	}
