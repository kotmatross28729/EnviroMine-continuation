...
public class Earthquake {
    ...
    ArrayList<int[]> ravineMask = new ArrayList<int[]>(); // 2D array containing x,z coordinates of blocks within the
                                                          // ravine
            ...
            if (EnviroMine.proxy.isClient()) {
                if (!(this instanceof ClientQuake)) {
                    ...
                    EnviroMine.instance.network.sendToAllAround(
                        ...
                        new TargetPoint(world.provider.dimensionId, posX, passY, posZ, 128 + size));
                }
            }
        }
    }
        ...
        while (passY < 256) {
            for (int i = 0; i < ravineMask.size(); i++) {
                ...
                for (int yy = y; yy >= 1; yy--) {
                    if ((world.getBlock(x, yy, z)
                        .getMaterial() == Material.lava && yy > 10) || world.getBlock(x, yy, z)
                            .getMaterial() == Material.water
                        || world.getBlock(x, yy, z)
                        ...
                        || (yy <= 10 && world.getBlock(x, yy, z)
                            .getMaterial() == Material.air)) {
                        if (world.getBlock(x, yy, z)
                            .getBlockHardness(world, x, yy, z) < 0) {
                            continue;
                        }
                        ...
                        if (yy <= 10) {
                            if (world.getBlock(x, yy, z)
                                ...
                                || world.getBlock(x, yy, z)
                                    .getMaterial() == Material.ground) {
                                // world.playSoundEffect(x, yy, z, "enviromine:cave_in", 1.0F, world.rand.nextFloat() *
                                // 0.5F + 0.75F);
                            }
                ...
                if ((chunk != null && chunk.getSavedLightValue(EnumSkyBlock.Sky, x & 0xf, y, z & 0xf) >= 15)
                    || world.getTopSolidOrLiquidBlock(x, z) < 16
        ...
        for (int y = 1; y < world.getActualHeight(); y++) {
            for (int i = 0; i < this.ravineMask.size(); i++) {
                ...
                if ((world.getBlock(x, y, z)
                    .getMaterial() == Material.lava && y > 10) || world.getBlock(x, y, z)
                        .getMaterial() == Material.water
    ...
    public static void updateEarthquakes() {
        ...
        for (int i = pendingQuakes.size() - 1; i >= 0; i--) {
            ...
            if (!quake.removeBlock() || quake.ravineMask.size() <= 0) {
                ...
                EnviroMine.instance.network.sendToAllAround(
                    new PacketEnviroMine(pData),
                    new TargetPoint(quake.world.provider.dimensionId, quake.posX, quake.passY, quake.posZ, 128 + size));
                pendingQuakes.remove(i);
            }
        }
    }
    ...
    public static void TickDay(World world) {
        if (world.rand.nextInt(2) == 0 && world.playerEntities.size() > 0) {
            ...
            // Chunk check can be disabled but may cause a large amount of chunks to be generated where the earthquake
            // passes through
            if (world.getChunkProvider()
                .chunkExists(posX >> 4, posZ >> 4)) {
