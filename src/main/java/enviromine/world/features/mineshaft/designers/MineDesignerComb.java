...
public class MineDesignerComb extends MineDesigner {
    ...
    public void StartDesign(MineshaftBuilder builder, int size, int depth, int auxSpacing) {
        int length = builder.rand.nextInt(128) + 128;

        for (int i = 0; i <= length; i++) {
            MineSegment baseSeg = new MineSegmentNormal(
                builder.world,
                builder.xOffset(i * 4, 0),
                depth,
                builder.zOffset(i * 4, 0),
                builder.rot,
                builder,
                true);
            baseSeg.setDecay(builder.decayAmount);
            baseSeg.linkChunksToBuilder();

            if (i % (auxSpacing + 1) == 0) {
                int lWidth = builder.rand.nextInt(8) + 8;

                for (int il = 1; il <= lWidth; il++) {
                    MineSegment segment = new MineSegmentNormal(
                        builder.world,
                        builder.xOffset(i * 4, il * 4),
                        depth,
                        builder.zOffset(i * 4, il * 4),
                        builder.rot,
                        builder,
                        true);
