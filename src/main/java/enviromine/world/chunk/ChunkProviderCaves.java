...
public class ChunkProviderCaves implements IChunkProvider
{
	...
	public ChunkProviderCaves(World worldIn, long seed)
	{
	...
	// prepareHeights
	public void func_147419_a(int noise_x, int noise_z, Block[] blockList, byte[] metaList)
	{
		byte b0 = 4;
		...
		this.noiseField = this.initializeNoiseField(this.noiseField, 
				noise_x * b0, 0, noise_z * b0, // Positions
				k, size_y, l // Size
				);
		...
		for(int i1 = 0; i1 < b0; ++i1)
		{
			for(int j1 = 0; j1 < b0; ++j1)
			{
				...
				{
					double d0 = 0.125D;
					double d1 = this.noiseField[((i1 + 0) * l + j1 + 0) * size_y + k1 + 0];
					double d2 = this.noiseField[((i1 + 0) * l + j1 + 1) * size_y + k1 + 0];
					double d3 = this.noiseField[((i1 + 1) * l + j1 + 0) * size_y + k1 + 0];
					double d4 = this.noiseField[((i1 + 1) * l + j1 + 1) * size_y + k1 + 0];
					double d5 = (this.noiseField[((i1 + 0) * l + j1 + 0) * size_y + k1 + 1] - d1) * d0;
					double d6 = (this.noiseField[((i1 + 0) * l + j1 + 1) * size_y + k1 + 1] - d2) * d0;
					double d7 = (this.noiseField[((i1 + 1) * l + j1 + 0) * size_y + k1 + 1] - d3) * d0;
					double d8 = (this.noiseField[((i1 + 1) * l + j1 + 1) * size_y + k1 + 1] - d4) * d0;
					...
					for(int l1 = 0; l1 < 8; ++l1)
					{
						...
						for(int i2 = 0; i2 < 4; ++i2)
						{
							...
							for(int k2 = 0; k2 < 4; ++k2)
							{
								...
								if(d15 > 0.0D)
								{
									block = CaveBaseProperties.caveStone_block;
									meta = CaveBaseProperties.caveStone_meta;
								}
								
								blockList[j2 += short1] = block;
								metaList[j2] = meta;
								//j2 += short1;
								d15 += d16;
							}
	...
	public void replaceBiomeBlocks(int chunkX, int chunkZ, Block[] blockList, byte[] metaList, BiomeGenBase[] biomes)
	{
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(this, chunkX, chunkZ, blockList, metaList, biomes, this.worldObj);
		...
		for(int k = 0; k < 16; ++k)
		{
			for(int l = 0; l < 16; ++l)
			{
				boolean apply_slowsand = this.slowsandNoise[k + l * 16] + this.hellRNG.nextDouble() * 0.2D > 0.0D;
				boolean apply_gravel = this.gravelNoise[k + l * 16] + this.hellRNG.nextDouble() * 0.2D > 0.0D;
				int i1 = (int)(this.netherrackExclusivityNoise[k + l * 16] / 3.0D + 3.0D + this.hellRNG.nextDouble() * 0.25D);
				int j1 = -1;
				
				Block block = CaveBaseProperties.caveStone_block;
				byte meta = CaveBaseProperties.caveStone_meta;
				Block block1 = CaveBaseProperties.caveStone_block;
				byte meta1 = CaveBaseProperties.caveStone_meta;
				...
				for(int k1 = 255; k1 >= 0; --k1)
				{
					...
					{
						Block block2 = blockList[l1];
						int meta2 = metaList[l1];
						
						if(block2 != null && block2.getMaterial() != Material.air)
						{
							if(block2 == CaveBaseProperties.caveStone_block && meta2 == CaveBaseProperties.caveStone_meta)
							{
								if(j1 == -1)
								{
									if(i1 <= 0)
									{
										block = null; meta = 0;
										block1 = CaveBaseProperties.caveStone_block; meta1 = CaveBaseProperties.caveStone_meta;
									}
									else if(k1 >= b0 - 4 && k1 <= b0 + 1)
									{
										block = CaveBaseProperties.caveStone_block; meta = CaveBaseProperties.caveStone_meta;
										block1 = CaveBaseProperties.caveStone_block; meta1 = CaveBaseProperties.caveStone_meta;
										
										if(apply_gravel)
										{
											block = CaveBaseProperties.caveGravel_block; meta = CaveBaseProperties.caveGravel_meta;
											block1 = CaveBaseProperties.caveStone_block; meta1 = CaveBaseProperties.caveStone_meta;
										}
										
										if(apply_slowsand)
										{
											block = CaveBaseProperties.caveDirt_block; meta = CaveBaseProperties.caveDirt_meta;
											block1 = CaveBaseProperties.caveDirt_block; meta1 = CaveBaseProperties.caveDirt_meta;
										}
									}
	...
	public Chunk provideChunk(int chunkX, int chunkZ)
	{
		...
		BiomeGenBase[] abiomegenbase = this.worldObj.getWorldChunkManager().loadBlockGeneratorData((BiomeGenBase[])null, chunkX * 16, chunkZ * 16, 16, 16); //Forge Move up to allow for passing to replaceBiomeBlocks
		this.func_147419_a(chunkX, chunkZ, ablock, meta);
		
		this.replaceBiomeBlocks(chunkX, chunkZ, ablock, meta, abiomegenbase);
		this.netherCaveGenerator.func_151539_a(this, this.worldObj, (long)chunkX * 341873128712L + (long)chunkZ * 132897987541L, chunkX, chunkZ, ablock);
		this.ravineGenerator.func_151539_a(this, this.worldObj, (long)chunkX * 341873128712L + (long)chunkZ * 132897987541L, chunkX, chunkZ, ablock);
