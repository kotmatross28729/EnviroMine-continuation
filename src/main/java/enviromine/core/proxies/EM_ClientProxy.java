        ...
        try {
            ...
            if (Loader.isModLoaded("RenderPlayerAPI")) {
                ...
                if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                    Level.WARN,
                    "Enviromine Doesn't support Player-API/Render-API yet! Config setting \"Render 3D Gear\" set to false");
            ...
            if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger.log(
                Level.ERROR,
                "Tried to Render Enviromine Gear, but Failed! Known issues with Render Player API.:- " + e);
        }
        ...
        while (tmp.hasNext()) {
            ...
            if (itemArmor instanceof ItemArmor && ((ItemArmor) itemArmor).armorType == 1) {
                ...
                if (EM_Settings.armorProperties.containsKey(name)
                    ...
                    IItemRenderer iRenderer = MinecraftForgeClient
                        .getItemRenderer(new ItemStack((ItemArmor) itemArmor), ItemRenderType.INVENTORY);

                    if (iRenderer != null) {
                        if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger
                            .log(Level.WARN, "Item " + name + " aready has a custom ItemRenderer associated with it!");
                        if (EM_Settings.loggerVerbosity >= EnumLogVerbosity.NORMAL.getLevel()) EnviroMine.logger
                            .log(Level.WARN, "EnviroMine will be unable to overlay the camel pack UI when attached!");
                    } else {
