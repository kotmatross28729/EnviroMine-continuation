...
public class EnviroMine {
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        event.getModMetadata().name = EnumChatFormatting.AQUA + EM_Settings.MOD_NAME;

        event.getModMetadata().version = EnumChatFormatting.DARK_AQUA + EM_Settings.VERSION;

        event.getModMetadata().credits = EnumChatFormatting.AQUA + "See Acknowledgments.txt";

        event.getModMetadata().authorList.clear();
        event.getModMetadata().authorList.add(
            EnumChatFormatting.BLUE + "Funwayguy, TimbuckTato, GenDeathrow, thislooksfun, AstroTibs, "
                + EnumChatFormatting.AQUA
                + "Kot"
                + EnumChatFormatting.GOLD
                + "matross");

        event.getModMetadata().url = EnumChatFormatting.GRAY + EM_Settings.URL;

        event.getModMetadata().description = EnumChatFormatting.GREEN
        ...
        if (Loader.isModLoaded("Thaumcraft")) isTCLoaded = true;
        if (Loader.isModLoaded("sereneseasons")) isSereneSeasonsLoaded = true;
        if (Loader.isModLoaded("etfuturum")) isEtFuturumLoaded = true;
        if (Loader.isModLoaded("Mekanism")) isMCELoaded = true;
        if (Loader.isModLoaded("lotr")) isLOTRLoaded = true;
        if (Loader.isModLoaded("cookingforblockheads")) isCFBHLoaded = true;
        if (Loader.isModLoaded("cfm")) isCFMLoaded = true;
        if (Loader.isModLoaded("netherlicious")) isNetherliciousLoaded = true;
        if (Loader.isModLoaded("harvestcraft")) isHarvestCraftLoaded = true;
        if (Loader.isModLoaded("jaffa")) isJAFFALoaded = true;
        if (Loader.isModLoaded("endlessids")) isEndlessIDsLoaded = true;
        if (Loader.isModLoaded("Growthcraft")) isGrowthcraftLoaded = true;
        ...
        if (EM_Settings.shaftGen) {
            VillagerRegistry.instance()
                .registerVillageCreationHandler(new EnviroShaftCreationHandler());
